# Jython Monthly / Newsletters / November 2006
...
Then there\'s also the recent release of [IronPython](./IronPython.html) by Microsoft via Jim Hugunin, the original creator of Jython. [IronPython](./IronPython.html) has proven that Python can run even faster on a VM, so the logical next step would be to bring Jython up to speed as well. We certainly don\'t want the JVM\'s Python story to be lacking.
...
What would be the next major steps for Jython if we wanted to start moving it back toward Python compatibility and [IronPython](./IronPython.html) performance levels?
...
[PyDev 1.2.5 Released](http://pydev.sourceforge.net/)
...
[Using Python Interpreter](http://ivarsv.blogspot.com/2006/10/python.html)
