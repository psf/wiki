...
You can trick git by using shell. The following example works albeit a bit slow:
...
But it doesn\'t work if a user wants to provide a list of branches to push: the command `git push-to-all-remotes master` is expanded by git as `!git remote | xargs -n1 git push master` which is certainly not what the user wants \-- remote\'s name must comes first, before branches. This is a fix:

    [alias]
    push-to-all-remotes = !git remote | xargs -I% -n1 git push %

Then the command `git push-to-all-remotes master` is expanded by git as `!git remote | xargs -I% -n1 git push % master`; xargs substitutes `%` with remote\'s name.
:::
:::::
