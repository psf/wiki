...
3.  Give that to [PyInstaller](../../people/PyInstaller) trunk with latest UPX (with LZMA support) and its\'s done.

Then, you might want to see the output of ArchiveViewer.py run over the final executable, so that you will see the list of all modules that have been brought in. If there\'s something that [PyInstaller](../../people/PyInstaller) thought you might need but you actually don\'t use, you can hand-edit the `excludes=[]` list in the .spec file to avoid bringing them in.
