...
There, that\'s the essence of my too-small-for-a-PEP talk! [MartinManey](../people/MartinManey)
...
Funny, I thought I was going to talk about improvements to the docs here. Maybe next time! \-- [MartinManey](../people/MartinManey)

It seems to me defaults are just place holders, what I want is to check for file.ini and if it is not found in the current directory write it with defaults. There is no \'defaults\' section, if I want defaults I just delete the file.ini. \-- [GaryMoffatt](../people/GaryMoffatt)
...
- Very simple: the order isn\'t stored; the sections are stored as a dictionary. It\'s also possible that the file contains multiple sections by the same name, which are collapsed into a single entry in the internal dictionary. \--[FredDrake](../people/FredDrake)
  ...
  - Thanks. I had thought that .INI files had to have unique sections, but perhaps not. Being order-preserving allows sections to have precedence based on order, rather than an explicit attribute (which is what the config file I\'ve inherited has done, although it was parsed using another language\'s INI module). \-- [StephenHahn](./StephenHahn.html)
    ...
    - Hmm. Other parsers may be less forgiving. Too bad there\'s no real specification for the INI format (as best as I\'ve been able to tell). \--[FredDrake](../people/FredDrake)
