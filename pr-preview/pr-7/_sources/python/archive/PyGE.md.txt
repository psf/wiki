...
I think a better solution for PyGE users would be for me (or other interested users) to perform a database update on the Gutenberg site at regular intervals (such as a couple of updates per month) and post the latest updates on the [SourceForge](../people/SourceForge) site for download. Then the PyGETS application could be used to download the latest e-texts in various formats without the average user ever having to invoke the lengthy acquire function.
