# Python Developers

```{admonition} Legacy Wiki Page
:class: note

This page was migrated from the old MoinMoin-based wiki. Information may be outdated or no longer applicable. For current documentation, see [python.org](https://www.python.org).
```

Stuff of interest to developers:

- [PythonThreeThousand](PythonThreeThousand)

- [CvsToSvn](CvsToSvn)

- [PythonTesters](PythonTesters)

- [Python Developer Global Job Trends](http://www.odesk.com/trends/python)

A list of developers using Python:

- [MichaelWax](../people/MichaelWax)

- [thanos](./thanos.html) - NYSE Python developer

- Vishnu Nath

- [CharlesThayer](../people/CharlesThayer) - Writing Systems Software, Bay Area

- [TomBryan](../people/TomBryan)

- [MarkHughes](../people/MarkHughes) - author of the [Umbra CRPG](http://kuoi.asui.uidaho.edu/~kamikaze/Umbra/), written in Python

- [JarekZgoda](../people/JarekZgoda)

- [PhippsXue](../people/PhippsXue)

- [aKKu](./aKKu.html)

- [Vasudev Ram - Dancing Bison Enterprises](http://wiki.python.org/moin/VasudevRam) - author of the [xtopdf toolkit](http://www.dancingbison.com/products.html), written in Python.
