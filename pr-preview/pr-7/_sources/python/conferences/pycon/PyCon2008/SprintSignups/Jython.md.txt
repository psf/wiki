# PyCon 2008 / Sprint Signups / Jython

```{admonition} Legacy Wiki Page
:class: note

This page was migrated from the old MoinMoin-based wiki. Information may be outdated or no longer applicable. For current documentation, see [python.org](https://www.python.org).
```

## Sign Up for a Sprint!

Are you interested in sprinting in this project? Great!

Please add your name below, include your email address.

If you\'re not sure which project to join, you can register in the [generic sprint sign-up page](http://wiki.python.org/moin/PyCon2008/SprintSignups).

See you at PyCon!

**Project Info**

Jython is an implementation of the high-level, dynamic, object-oriented language Python seamlessly integrated with the Java platform.

Project URL: [http://www.jython.org](http://www.jython.org)

Sprint Leader: Frank Wierzbicki

Code repository and developer info: [http://wiki.python.org/jython/JythonDeveloperGuide](http://wiki.python.org/jython/JythonDeveloperGuide)

New contributor info:

- developer faq: [http://jython.org/Project/devfaq.html](http://jython.org/Project/devfaq.html)
- developer guide: [http://wiki.python.org/jython/JythonDeveloperGuide](http://wiki.python.org/jython/JythonDeveloperGuide)
- mailing lists: [http://sourceforge.net/mail/?group_id=12867](http://sourceforge.net/mail/?group_id=12867)

**Sprinters**

- Frank Wierzbicki (fwierzbicki at gmail.com)
- Tobias Ivarsson (thobes at gmail.com)
- Jim Baker (jbaker at zyasoft.com)
- Steven Githens (sgithens at caret.cam.ac.uk)
- Dave Gourley (davidigourley at gmail.com)
- Oti Humbel (ohumbel at gmail.com; GMT+1)
- Nicholas Riley (njriley at uiuc.edu)
