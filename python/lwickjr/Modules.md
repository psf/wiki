...
My more-interesting modules include \"[Alias](./lwickjr(2f)Modules(2f)Modules(2f)Alias.html){.nonexistent}\", \"[Edit](./lwickjr(2f)Modules(2f)Modules(2f)Edit.html){.nonexistent}\", and \"[UT](./lwickjr(2f)Modules(2f)Modules(2f)UT.html){.nonexistent}\". I\'ll add others to this page as I have opportunity.

# General Outlines {#General_Outlines}

- [Alias.py](attachments/lwickjr(2f)Modules/Alias.py){.attachment} is a convenience module for I.D.L.E. [PyShell](./PyShell.html){.nonexistent} usage, intended to save typing.
...
- [Edit.py](attachments/lwickjr(2f)Modules/Edit.py){.attachment}
...
It seems to me that Edit is something that makes it so that you can jump straight to where a variable is defined, in IDLE. (Not really sure.) If so, that\'s awesome! I\'ve often times wished IDLE were more powerful; This is the kind of thing I\'d like to see more of. ![:)](/wiki/europython/img/smile.png ":)"){height="16" width="16"}
...
- [UT.py]( "Upload new attachment "UT.py""){.attachment .nonexistent} - last uploaded 2005-11-26 - is a convenience module for miscellaneous support functions that I\`ve found useful.
  ...
  - [ThisLine](./ThisLine.html){.nonexistent}, a function that returns the name and source line-number of the caller an arbitrary number of callers up the call-stack.

  - Some old stuff, commented out.

------------------------------------------------------------------------

- [GWiz.py](attachments/lwickjr(2f)Modules/GWiz.py){.attachment}, a \"\"functional\"\" but incomplete GUI editor. More details later.

------------------------------------------------------------------------

- [WinReg.py]( "Upload new attachment "WinReg.py""){.attachment .nonexistent}, a higher-level abstraction around \_winreg. Fully functional and complete.
