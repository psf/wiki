...
PyjamasDesktop is a cross-platform framework and applications widget set for the desktop. There are three back-ends: [WebKit](http://webkit.org){.http}, [python-xpcom](http://pyxpcomext.mozdev.org){.http} and MSHTML. The XULrunner port uses python-xpcom and hulahop; the Webkit port uses [PyWebkitGtk](PyWebkitGtk) and the MSHTML port uses python win32 comtypes. PyjamasDesktop is similar to [KiWi](http://www.async.com.br/projects/kiwi){.http} in that it provides an easy-to-use API, where you need not know - at all - that you are running PyGTK, XULrunner or MSHTML underneath.

PyjamasDesktop is actually a port of [PyJamas](http://code.google.com/p/pyjamas){.http} which is a web toolkit, so you also have the option of running PyjamasDesktop applications as web applications - unmodified.
...
- Full SVG Canvas support, like [HippoCampus](./HippoCampus.html){.nonexistent}, if the web engine supports it (currently only XULRunner)
...
- [PyPi Pyjamas](http://pypi.python.org/pypi/Pyjamas){.http}

- [webkit issue 16401](https://bugs.webkit.org/show_bug.cgi?id=16401){.https}

- [pywebkitgtk issue 13](http://code.google.com/p/pywebkitgtk/issues/detail?id=13){.http}

Pre-built AMD64 .deb packages for the webkit-glib and pywebkitgtk dependencies are here:

- [Packages for pywebkitgtk-1.0 and libwebkit-1.0-2](http://sourceforge.net/project/showfiles.php?group_id=236659){.http}
...
- [API Documentation](http://pyjs.org/api/){.http}

Article and information is available here:

- [advogato article](http://advogato.org/article/981.html){.http}

- [http://pyjs.org](http://pyjs.org){.http}
