...
## PyConDC 2004 Sprint plan {#PyConDC_2004_Sprint_plan}
...
### Sprints Being Planned for PyConDC2004 {#Sprints_Being_Planned_for_PyConDC2004}
...
- [Zope2Sprint](Zope2Sprint) \-- Coach: Chris [McDonough](./McDonough.html){.nonexistent} \-- Not certain what to sprint on here, but a topic should come up soon. Recommended topics so far have been: bugathon, improved ZODB \"blob\" support, and documentation.
...
- Jython sprint \-- work on some of the \"Jython should be fixed\" items from the [Jython website](http://www.jython.org/docs/differences.html){.http}

- [DistutilsDependencies](./DistutilsDependencies.html){.nonexistent} \-- implement a system to allow package authors to specify their dependencies, and have them automatically downloaded and installed when a user installs the package that needs the dependencies

- Distutils sprint \--
  - Finish the implementation of PEP262\'s [InstallationDatabase](./InstallationDatabase.html){.nonexistent} class and have test cases for it.
  ...
  - Make any PyPI changes that are required (to a test installation, not the live one on python.org).
    ...
    A related sprint will likely tackle [DistutilsDependencies](./DistutilsDependencies.html){.nonexistent}.

### Willing Participants Without a Particular Sprint Yet {#Willing_Participants_Without_a_Particular_Sprint_Yet}
...
- Fred Drake - Docutils, [DistutilsDependencies](./DistutilsDependencies.html){.nonexistent}, distutils
...
The [Python Software Foundation](http://www.python.org/psf){.http} is sponsoring the sprints for PyConDC2004. Please consider making a [donation](http://www.python.org/psf/donations.html){.http} to support this vital community activity!
:::
