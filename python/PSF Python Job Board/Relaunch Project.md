...
# PSF Python Job Board: Relaunch Project {#PSF_Python_Job_Board:_Relaunch_Project}
...
At the same time, we\'d like to move the job board from the legacy site to the new relaunched [http://www.python.org/](http://www.python.org/){.http} Django based site.

::: table-of-contents
Contents

1.  [PSF Python Job Board: Relaunch Project](#PSF_Python_Job_Board:_Relaunch_Project)
    1.  [Old job board process](#Old_job_board_process)
    2.  [New job board process](#New_job_board_process)
    3.  [Jobs app design](#Jobs_app_design)
        1.  [Job Model Reference](#Job_Model_Reference)
    4.  [Project organization](#Project_organization)
        1.  [Roles](#Roles)
        2.  [Mailing list](#Mailing_list)
        3.  [Issue tracker](#Issue_tracker)
    5.  [Resources](#Resources)
        1.  [New job board](#New_job_board)
        2.  [Old job board](#Old_job_board)
        3.  [Web site administration](#Web_site_administration)
        4.  [Web site code base](#Web_site_code_base)
:::

## Old job board process {#Old_job_board_process}
...
1.  Job submitters created a job posting based on the template listed on the [Job board how-to page](http://legacy.python.org/community/jobs/howto/){.http}
...
## New job board process {#New_job_board_process}
...
## Jobs app design {#Jobs_app_design}
...
### Job Model Reference {#Job_Model_Reference}

Notes on how various field in the database should be updated:

- **Region**: If not applicable, set to the same thing as \"Country\", until this is no longer a required field. ([GitHub Issue](https://github.com/python/pythondotorg/issues/271){.https})
...
The list gets all email sent to [jobs@python.org](mailto:jobs@python.org){.mailto}. The old process used this email address for job submissions. The new process will use a web form based approach instead.
...
- [python.org site\'s issue tracker](https://github.com/python/pythondotorg/issues){.https}
...
### New job board {#New_job_board}

- [Job board temporary installation](http://www.python.org/newjobs/){.http} on the production site. This is a temporary location of the new Python job board, run using Django and backed by a database.

  ![/!\\](/wiki/europython/img/alert.png "/!\"){height="16" width="16"} Please note that the database used by this temporary installation is the production one, so any changes to the database contents should be done with care!

- [Staging system implementation of the new job board](https://staging.python.org/newjobs/){.https} This runs a copy of the production database on the staging branch of the python.org repository.

- [New job board](http://www.python.org/jobs/){.http} This will be the production location of the Python job board.
...
- [Python Jobs Board Github Repository](https://github.com/python-jobs-board/pythondotorg){.https} We were using this for the jobs app development. It\'s a fork of the main python.org repo.

- [Python Jobs Board Issue Tracker](https://github.com/python-jobs-board/pythondotorg/issues?state=open){.https} This is the tracker we were using for the jobs app development. Development has now (2015) moved back to the main python.org repository.

### Old job board {#Old_job_board}

- [Job board listing ported from the legacy site](http://www.python.org/community/jobs/){.http} This is a single page which has to be manually maintained via the Django admin interface.

- [Legacy job board site](http://legacy.python.org/community/jobs/){.http} This page was manually maintained through the Subversion repository which runs the legacy website.

- [Job board how-to explaining the process that was used for the legacy site](http://legacy.python.org/community/jobs/howto/){.http}

### Web site administration {#Web_site_administration}

- [Django admin interface of the new site](https://www.python.org/admin/){.https}
...
- [Github repository of the new site](https://github.com/python/pythondotorg){.https}
