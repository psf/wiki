...
- Sponsorship - There are a number of companies (some international and some local) in India which use Python. An incomplete list is available [here](http://wiki.python.org/moin/BangPypers/PythonCompaniesInIndia){.http}. We plan to create a persuasive sponsorship kit and visit these organisations for funding.
...
quality presentations lined up, the event will suffer. This is one of the reasons why we wish to use the [PyCon](PyCon) brand name to promote the conference.

## Suggestions {#Suggestions}

- For future events (maybe 2010/11(?))

- Alternatively [PyCon](PyCon)-India could be held multiple times in a year, in each city which finds enough volunteers (read critical mass) to get
...
- **Seeking Sponsors**. For e.g, from Google, HP, [ThoughtWorks](./ThoughtWorks.html){.nonexistent}
