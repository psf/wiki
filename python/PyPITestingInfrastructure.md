...
1.  [Pypi Testing Infrastructure](#Pypi_Testing_Infrastructure)
    1.  [Gsoc 2011](#Gsoc_2011)
        1.  [General designs](#General_designs)
            1.  [Terminology](#Terminology)
            2.  [Environment part](#Environment_part)
                1.  [Architecture](#Architecture)
                2.  [Raw Data API](#Raw_Data_API)
                3.  [Command API](#Command_API)
                4.  [Slave](#Slave)
                5.  [Master](#Master)
            3.  [Execution part](#Execution_part)
                1.  [Detailed work](#Detailed_work)
                2.  [Global Architecture](#Global_Architecture)
                3.  [Execution manager](#Execution_manager)
                4.  [Essential tasks](#Essential_tasks)
                5.  [Common tasks](#Common_tasks)
        2.  [existing solutions](#existing_solutions)
            1.  [Master Slave Architecture](#Master_Slave_Architecture)
                1.  [Buildbot](#Buildbot)
                2.  [Bitten](#Bitten)
                3.  [Condor](#Condor)
                4.  [Findings](#Findings)
            2.  [Task Manager](#Task_Manager)
                1.  [Concepts](#Concepts)
                2.  [Pony-build](#Pony-build)
                3.  [Score](#Score)
                4.  [Apycot/narval](#Apycot.2Fnarval)
                5.  [Score](#Score-1)
                6.  [Buildbot](#Buildbot-1)
                7.  [Score](#Score-2)
                8.  [PythonTasks](#PythonTasks)
                9.  [Score](#Score-3)
            3.  [Dependent part](#Dependent_part)
                1.  [Setup API](#Setup_API)
                2.  [Raw Data API](#Raw_Data_API-1)
            4.  [Tracing library](#Tracing_library)
            5.  [Comparison of VMs](#Comparison_of_VMs)
            6.  [VM Handling](#VM_Handling)
        3.  [weekly meeting report](#weekly_meeting_report)
            1.  [2012-05-12](#A2012-05-12)
                1.  [Blog](#Blog)
                2.  [IRC](#IRC)
                3.  [Mailing List](#Mailing_List)
                4.  [Source](#Source)
                5.  [Python Version](#Python_Version)
                6.  [Wiki](#Wiki)
                7.  [Weekly Meeting](#Weekly_Meeting)
            2.  [2012-05-31](#A2012-05-31)
                1.  [Interaction Between Slave and VM](#Interaction_Between_Slave_and_VM)
                2.  [Overall Architecture](#Overall_Architecture)
                3.  [Focus for the near future](#Focus_for_the_near_future)
                4.  [For the next week (Yeswanth)](#For_the_next_week_.28Yeswanth.29)
                5.  [For the next week (Boris)](#For_the_next_week_.28Boris.29)
:::

# Pypi Testing Infrastructure {#Pypi_Testing_Infrastructure}
...
- Yeswanth: [http://advencode.wordpress.com/](http://advencode.wordpress.com/){.http}

- Boris: [http://feldboris.alwaysdata.net/blog/](http://feldboris.alwaysdata.net/blog/){.http}

### General designs {#General_designs}
...
#### Environment part {#Environment_part}
...
#### Execution part {#Execution_part}

##### Detailed work {#Detailed_work}
...
#### Master Slave Architecture {#Master_Slave_Architecture}
...
- Fine Configuration presents in [BuildBot](BuildBot), when you configure a task you can configure how to catch failure and warnings : [http://buildbot.net/buildbot/docs/latest/Common-Parameters.html#Common-Parameters](http://buildbot.net/buildbot/docs/latest/Common-Parameters.html#Common-Parameters){.http}.

##### Pony-build {#Pony-build}
...
[PythonTasks](./PythonTasks.html){.nonexistent} is one of boris\'s project, it\'s still in beta and no doc is written for the moment. Characteristics:

- Dependencies is either direct dependencies (task A depends on ![B)](/wiki/europython/img/smile2.png "B)"){height="16" width="16"} or output dependencies (task A need D input, B produces D, B must be executed before A). For the output dependencies mode, some code must be written as it will not be part (for the moment) of [PythonTasks](./PythonTasks.html){.nonexistent}.
...
- For the format , we both agreed on jsonrpc ([http://groups.google.com/group/json-rpc/web/json-rpc-2-0](http://groups.google.com/group/json-rpc/web/json-rpc-2-0){.http}).
...
#### Tracing library {#Tracing_library}

- faulthandler : Python\'s module to catch signals (sigsegv ie) and output them in stderr or in a file. Easy to parse the output but trace only signals which is already traced by others solution. ([http://blog.python.org/2011/05/new-faulthandler-module-in-python-33.html](http://blog.python.org/2011/05/new-faulthandler-module-in-python-33.html){.http}).

#### Comparison of VMs {#Comparison_of_VMs}

::: {}
  -------------------------------- ------------------------------------------------- ------------------------------------------------------ ---------------------------------------------------- -----------------------------------------------
  Criteria                         Xen                                               Qemu+KQemu                                             KVM                                                  [VirtualBox](./VirtualBox.html){.nonexistent}
...
So [VirtualBox](./VirtualBox.html){.nonexistent} to be used.

#### VM Handling {#VM_Handling}
...
### weekly meeting report {#weekly_meeting_report}

#### 2012-05-12 {#A2012-05-12}

##### Blog {#Blog}

- (update once in a week, and whenever there is something to say)
  - Yeswanth - [http://www.advencode.wordpress.com](http://www.advencode.wordpress.com){.http} Boris - [http://feldboris.alwaysdata.net/blog/](http://feldboris.alwaysdata.net/blog/){.http}
...
- General Mailing List : [pyti@librelist.com](mailto:pyti@librelist.com){.mailto} Specific : distutils-sig / python-testing / catalog-sig / python-dev depending the cases

##### Source {#Source}

- Vanilla Mercurial + [BitBucket](./BitBucket.html){.nonexistent}
...
- [http://wiki.python.org/moin/PyPITestingInfrastructure](http://wiki.python.org/moin/PyPITestingInfrastructure){.http}

##### Weekly Meeting {#Weekly_Meeting}

For the next week(Boris): Code : Finish some little things on [PyTasks](./PyTasks.html){.nonexistent} (need it for a project) and compare it to another task manager included in tools like waff and narval (not sure about pony-build) Design : Will read some of my bookmarks about distributed computing communication : Write a summary about task manager and about which protocol we can use for API

For the next week(Yeswanth):

Code : Not much this week( Will work on reading about python coding conventions) Other tools/design : Will go through condor or other equivalents and see how it fits in this project Communication : Report on condor ([http://www.cs.wisc.edu/condor/](http://www.cs.wisc.edu/condor/){.http}) or other equivalents

#### 2012-05-31 {#A2012-05-31}

##### Interaction Between Slave and VM {#Interaction_Between_Slave_and_VM}
...
##### Focus for the near future {#Focus_for_the_near_future}

- Managing VM slave side (i.e configuring and managing VM )
- Getting a Task Manager that runs task

##### For the next week (Yeswanth) {#For_the_next_week_.28Yeswanth.29}
...
##### For the next week (Boris) {#For_the_next_week_.28Boris.29}
