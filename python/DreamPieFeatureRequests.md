...
This page is intended as a place to suggest features for [DreamPie](http://dreampie.sourceforge.net/){.http}.

[DreamPie](http://dreampie.sourceforge.net/){.http} is a graphical interactive Python shell which is designed to be reliable and fun. Check it out at [http://dreampie.sourceforge.net/](http://dreampie.sourceforge.net/){.http}

## Richer function documentation {#Richer_function_documentation}
...
## Complete module names {#Complete_module_names}

This was a bug report by cool-RR: [https://bugs.launchpad.net/dreampie/+bug/525610](https://bugs.launchpad.net/dreampie/+bug/525610){.https}

IPython completes module names. That\'s a good idea.

bpython does this too.

## More by cool-RR: Complete things that aren\'t defined yet {#More_by_cool-RR:_Complete_things_that_aren.27t_defined_yet}
...
I\'d want [DreamPie](./DreamPie.html){.nonexistent} to autocomplete both the `whatever`{.backtick} thing and the use of `factorial`{.backtick} inside the definition. Probably hard, I know.
...
## Save code history between sessions {#Save_code_history_between_sessions}
...
## Pasting clipboard datas using the mouse middle button {#Pasting_clipboard_datas_using_the_mouse_middle_button}
...
Since in [DreamPie](./DreamPie.html){.nonexistent} the right mouse button is used by the context menu, it\'ll be good to have the same feature with the middle mouse button (or the wheel one).

## Setting the bottom box area height {#Setting_the_bottom_box_area_height}

The bottom box area have a fixed height at startup. Saving and restoring will be useful.

## Saving window position and size {#Saving_window_position_and_size}
...
## Notify new versions {#Notify_new_versions}

Let [DreamPie](./DreamPie.html){.nonexistent} check and notify if a new version is available at startup (or programmatically).

## Autocomplete keyword arguments {#Autocomplete_keyword_arguments}

Imagine I\'m typing this:

    x = sum(my_list, sta

In this case we have the `sum`{.backtick} function, which takes a keyword argument `start`{.backtick}. It\'d be nice if [DreamPie](./DreamPie.html){.nonexistent} could autocomplete the keyword argument itself. (Ram \"cool-RR\" Rachum.)

## Behavior of Ctrl+Something not good enough {#Behavior_of_Ctrl.2B-Something_not_good_enough}

I often use ctrl+arrow or ctrl+delete to move through or delete a word. In programmer-friendly editors (Like Wing, Eclipse, Aptana, etc.) this works great. In non-programmer friendly editors (like Notepad, IDLE (shamefully), or textareas in browsers), the cursor often jumps too far in one stroke. Unfortunately it happens in [DreamPie](./DreamPie.html){.nonexistent} as well.

## Display version in caption bar on Windows {#Display_version_in_caption_bar_on_Windows}

On Windows, [DreamPie](./DreamPie.html){.nonexistent} creates launch shortcuts for each version of Python on the system. This is great, because it makes [DreamPie](./DreamPie.html){.nonexistent} very useful for doing A/B comparisons between versions. It would be even better if the window caption contained the Python version\--for example, instead of just \"[DreamPie](./DreamPie.html){.nonexistent}\", \"[DreamPie](./DreamPie.html){.nonexistent} - Python 2.7\".

They don\'t automatically contain the version number? Because on my system, they do, and they should. Can you please post a bug report, with the version of [DreamPie](./DreamPie.html){.nonexistent} you\'re using? \--Noam

Done: [https://bugs.launchpad.net/dreampie/+bug/610160](https://bugs.launchpad.net/dreampie/+bug/610160){.https} \-- tlesher

## Highlight all occurrences of the word(token) under the caret {#Highlight_all_occurrences_of_the_word.28token.29_under_the_caret}
...
## Allow cycling through history without ctrl modifier {#Allow_cycling_through_history_without_ctrl_modifier}

Currently [DreamPie](./DreamPie.html){.nonexistent} requires the \'ctrl\' key along with up or down arrows to cycle through history. Perhaps this could be done with arrow keys alone like this:
...
## Let \'esc\' clear the editing window if in the middle of history {#Let_.27esc.27_clear_the_editing_window_if_in_the_middle_of_history}
...
## Snippet templates {#Snippet_templates}
...
## Allow bold fonts {#Allow_bold_fonts}

\--Rainer

## docstring shortcut {#docstring_shortcut}

shortcut to show the docstring of an object, like ? in IPython

## Line Numbers! {#Line_Numbers.21}

\--Ilia Zaslavsky

## Automatic code per python version {#Automatic_code_per_python_version}
...
## Two behaviours for path auto-completion {#Two_behaviours_for_path_auto-completion}
...
## Faster history searching using Ctrl-up/Ctrl-down {#Faster_history_searching_using_Ctrl-up.2FCtrl-down}
...
## Show tab and space characters symbols {#Show_tab_and_space_characters_symbols}
...
\-- [JamesThomasMoon1979](./JamesThomasMoon1979.html){.nonexistent}

## Enable manual removal of elements from history {#Enable_manual_removal_of_elements_from_history}
...
## Collapsed text/code in saved history {#Collapsed_text.2Fcode_in_saved_history}

When saving history as HTML, I would like to be able to expand/collapse collapsed text/code as in [DreamPie](./DreamPie.html){.nonexistent} itself.
:::
