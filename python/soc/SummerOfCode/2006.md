...
Repository: [http://svn.python.org/projects/python/branches/hoxworth-stdlib_logging-soc/](http://svn.python.org/projects/python/branches/hoxworth-stdlib_logging-soc/)

[Project Blog](http://www.jackilyn.com/category/google-soc/)
...
Repository: [http://svn.python.org/view/sandbox/trunk/decimal-c/](http://svn.python.org/view/sandbox/trunk/decimal-c/)
...
Repository: [https://svn.sourceforge.net/svnroot/ziparchive/ziparchive/trunk](https://svn.sourceforge.net/svnroot/ziparchive/ziparchive/trunk)
...
Repository: [http://codespeak.net/svn/pypy/dist/pypy/translator/js/](http://codespeak.net/svn/pypy/dist/pypy/translator/js/)

The main target of the project is to produce a framework which takes advantage of [PyPy](../../implementations/PyPy)\'s [JavaScript](./JavaScript.html) backend and to make it easy to write an AJAX based framework written entirely in Python.

The aim is to produce a framework which allows Python applications to be translated into an AJAX based application split into a client part ([JavaScript](./JavaScript.html) running on browser) and a server part (python running on web server). Communication between client and server is translated from Python method calls into JSON request and response.
...
Repository: [http://codespeak.net/svn/pypy/dist/pypy/translator/cli/](http://codespeak.net/svn/pypy/dist/pypy/translator/cli/)

gencli is the [PyPy](../../implementations/PyPy) backend targeting the Common Language Infrastructure (CLI) virtual machine, i.e. .NET.

Once gencli will be complete it will be possible to translate the Standard Interpreter in order to produce a .NET executable of the Python interpreter (we could call it [PyPy](../../implementations/PyPy).NET).

The result will be in some way similar to the existing [IronPython](../../implementations/IronPython), with the difference that [IronPython](../../implementations/IronPython) is a compiler (though most of the job is done by the runtime environment), while [PyPy](../../implementations/PyPy).NET will be an interpreter.
...
[http://codespeak.net/svn/user/rhymes/modules/](http://codespeak.net/svn/user/rhymes/modules/) [http://codespeak.net/svn/pypy/dist/pypy/module/](http://codespeak.net/svn/pypy/dist/pypy/module/)

I\'d like to make [PyPy](../../implementations/PyPy) ready for encrypted communications adding SSL support in the ongoing and in-development socket module. The other modules I\'ll port are: bz2, fcntl, mmap and time (very fundamental). If there\'s time I\'d like to start working on \'os\' and \'select\' modules also.
...
Repository: [http://svn.scipy.org/svn/scipy/trunk/Lib/sandbox/ann/](http://svn.scipy.org/svn/scipy/trunk/Lib/sandbox/ann/)
...
Repository: [http://svn.sourceforge.net/viewcvs.cgi/matplotlib/trunk/matplotlib/lib/matplotlib/](http://svn.sourceforge.net/viewcvs.cgi/matplotlib/trunk/matplotlib/lib/matplotlib/)
...
[Soya](http://www.soya3d.org/) lacks some very useful tools such as a collision API or a properly integrated physics engine.

I am applying to Summer of Code 2006 in order to fix these deficiencies by improving support for the [Open Dynamics Engine](http://www.ode.org/), which is barely working and too low-level in the last release of Soya, and provide a similar collision detection system to all objects whether ODE is in use or not.
...
Repository: [http://svn.pysoy.org/trunk/blender-soy](http://svn.pysoy.org/trunk/blender-soy)

Browse Source: [http://www.pysoy.org/browser/trunk/blender-soy](http://www.pysoy.org/browser/trunk/blender-soy)

Currently, no [PySoy](http://www.pysoy.org/) importers exist for Blender. This means you cannot import any Soy or Cal3D models into Blender. This can be inconvenient for both the game developer, as he needs to publish the .blend files to allow people to modify models, and for a gamer community surrounding a project, as they can\'t modify models unless the developer publishes the .blend files. (Note: This might be a perfectly okay and even preferable situation for some proprietary game developers, but [PySoy](../../people/PySoy) is built on a philosophy of openness and freedom, making this a more important feature.)

The [PySoy](../../people/PySoy) exporter tools basically consists of the blender2soya and blender2cal3d scripts. blender2soya is currently used for static (non-animated) models, and blender2cal3d is used for exporting animated skeletal models to the Cal3D format, which can also be used with [PySoy](../../people/PySoy). These two scripts lack many features, however, and still contain known flaws.
...
If I am assigned to this project, I will create [PySoy](http://www.pysoy.org/) and Cal3D importers for Blender, and I will work on both exporter scripts

*Due to an inconviently-timed project fork, this project has been moved to [PySoy](../../people/PySoy) (was formerly for Soya3D). Beside the name of the project, all aspects remain the same and the code will be available for the Soya3d project and any other project which needs it.*
...
[Project Home](http://code.google.com/p/grail-soc/)
...
Student: [Sean Handley](http://www.seanhandley.com)

Mentor: [Fabio Zadrozny](http://www.blogger.com/profile/4798968)

Repository: [http://pydev.cvs.sourceforge.net/pydev/](http://pydev.cvs.sourceforge.net/pydev/)

Project Blog: [http://www.planet-soc.com/?q=blog/114](http://www.planet-soc.com/?q=blog/114)

My objectives for [PyDev](../../people/PyDev) are to implement as many of the most popular requested features as I can, bringing the power of Python development in Eclipse up to the same standards as Java. Specifically, I will be adding new features to the debugger and code editor.
...
Repository: [http://svn.crunchy.python-hosting.com/branches/jwsoc](http://svn.crunchy.python-hosting.com/branches/jwsoc)
...
Repository: [https://svn.sourceforge.net/svnroot/mailman/branches/soc2006-webui](https://svn.sourceforge.net/svnroot/mailman/branches/soc2006-webui)
...
Repository: [http://svn.pyyaml.org/libyaml/trunk/](http://svn.pyyaml.org/libyaml/trunk/)

Timeline: [http://pyyaml.org/timeline](http://pyyaml.org/timeline)
...
The remaining time will be spent on resolving all problems that would occur during usage of Cheesecake and PyPI. Along with fixing bugs, I will develop a simple Hello world package that can be taken as an example of good development practices for all Python developers. It should also score 100% in the Cheesecake test of course. ![;-)](/wiki/europython/img/smile4.png%20";-)") It will be what [hello](http://savannah.gnu.org/projects/hello/) is for GNU Project.
...
Website: [http://pycells.pdxcb.net/](http://pycells.pdxcb.net/)

Repository: [http://pycells.pdxcb.net/svn](http://pycells.pdxcb.net/svn)
...
Student: [Marcin Pertek](http://zygfryd.voidfx.net/)

Mentor: Mark Hammond

Repository: [http://svn.pysoy.org/trunk/pymplug/](http://svn.pysoy.org/trunk/pymplug/)

Source Browser: [http://www.pysoy.org/browser/trunk/pymplug/](http://www.pysoy.org/browser/trunk/pymplug/)
...
In addition to ease the creation of plugins, I would make bindings to readily enable the use of wxPython, PyGTK, [PyGame](../../multimedia/PyGame) and Soya3D. These bindings would enable those libraries to handle the in-browser window and associated events.

## Support Vector Machines for SciPy 

Student: [Albert Strasheim](http://students.ee.sun.ac.za/~albert/)

Mentor: Dave Kammeyer

Repository: [http://pycheesecake.org/browser](http://pycheesecake.org/browser)

Integrating libsvm (and possibly other SVM libraries) with [SciPy](http://www.scipy.org/) using ctypes and providing various tools for manipulating datasets for use with SVMs.

[Project homepage](http://students.ee.sun.ac.za/~albert/pylibsvm/)
