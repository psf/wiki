...
Web page: [OPeNDAP/DODS module for Python](http://opendap.oceanografia.org/)
...
Web page: [Bitten](http://bitten.cmlenz.net)
...
Lots of good code, very good time management and prioritization, and he\'s delivered a working system. (Lots still to be done, of course, but it\'s up and running.) See [http://bitten.cmlenz.net](http://bitten.cmlenz.net) for details.
...
The project website, where you can also find the original proposal sent to Google, is at [http://openexvis.sourceforge.net/](http://openexvis.sourceforge.net/) and the progress during Summer of Code is tracked in Tero\'s blog at [http://www.teroajk.net/blog/](http://www.teroajk.net/blog/) .
...
Many additions to Wax were released in these two months, including WaxRF (a system to load forms from XML, much like wxPython\'s XRC), a documentation viewing/generating tool, and a number of new controls. Some existing issues were also fixed ([OverlaySizer](./OverlaySizer.html), Wizard). More information at [http://zephyrfalcon.org/weblog2/arch_e10_00810.html#e817](http://zephyrfalcon.org/weblog2/arch_e10_00810.html#e817).

## Data Serving/Collection Framework in Python/WSGI 

Student: Ho Chun Wei

Weblog: [http://cwho.blogspot.com/](http://cwho.blogspot.com/)
...
Web page: [PBNT -- Python Bayesian Network Toolbox](http://pbnt.berlios.de/)
...
For (almost) daily updates please see [http://elliotpbnt.blogspot.com](http://elliotpbnt.blogspot.com).
...
- Toni Alatalo, [http://an.org/programming](http://an.org/programming), Documnet transformation functionality.
...
Original proposal: [http://ipython.scipy.org/google_soc/ipnb_google_soc.pdf](http://ipython.scipy.org/google_soc/ipnb_google_soc.pdf).

Further details: [http://www.scipy.org/wikis/featurerequests/NoteBook](http://www.scipy.org/wikis/featurerequests/NoteBook)
...
svn co [http://ipython.scipy.org/svn/ipython/nbshell/trunk](http://ipython.scipy.org/svn/ipython/nbshell/trunk) nbshell
...
[http://projects.scipy.org/ipython/ipython](http://projects.scipy.org/ipython/ipython)

## Porting \_sre.c and arraymodule.c to Python 

Student: Niklaus Haldimann

Weblog: [http://ubique.ch/soc](http://ubique.ch/soc)

Mentors: Armin Rigo, Samuele Pedroni

### Proposal 

I would like to create a port of the standard library modules \"\_sre\" and \"array\" to pure Python. This will benefit alternative Python implementations like [PyPy](../../implementations/PyPy), Jython and [IronPython](../../implementations/IronPython). These projects all have to provide their own implementations of standard library modules written in C if they\'re not available in pure Python.

### Outcome 

Niklaus Haldimann has done everything planned and more on writing a pure Python implementation of the \_sre (core of regular expressions) and array modules ([http://codespeak.net/svn/user/nik](http://codespeak.net/svn/user/nik)). In addition to the base work, he made regular releases of the \_sre module (three so far). This is a drop-in replacement for the equivalent C modules; for example, \_sre.py could be used in projects that need to run on top of Python 2.3 or earlier, while using regexps that require the recursion-less approach introduced in 2.4 and supported by \_sre.py.

He has also integrated the modules with [PyPy](../../implementations/PyPy) (he was present at two [PyPy](../../implementations/PyPy) sprints); in the case of the \_sre module, he ported it to \"interpreter-level\", which means that in addition to integration, he made sure that the code is static enough to be automatically translatable to good C code, like the rest of [PyPy](../../implementations/PyPy). This should give an excellent write-once, translate-to-anything regular expression engine that might also be used as the basis for a Jython or [IronPython](../../implementations/IronPython) module (or even back to a C extension module for CPython).
...
Web page: [PyTrails](http://seul.org/~jennifer/proj/pytrail/)

### Proposal 

I\'m working on an extensible opensource engine for implementing trail-style games such as [Oregon Trail](http://www.gamespot.com/gamespot/features/all/greatestgames/p-34.html) or Amazon Trail. The primary goal is to produce a quality edutainment title that can be used free of cost. The secondary goal is to make it easy for other edutainment trail games to be created. [PyTrails](../../people/PyTrails) will be Python based and uses [PyGame](../../multimedia/PyGame). The engine will allow following a branching map including making stops to rest, hunt, or trade. Additional choices such as shopping and fording rivers may be available at special points. Each of these activities will be replacable in other trail games as to allow for maximum flexibility.
...
[The student\'s assessment](http://starship.python.net/crew/mwh/blog/nb.cgi/view/nickblog/2005/09/01/0)
...
Library releases available from: [http://developer.berlios.de/project/showfiles.php?group_id=4124](http://developer.berlios.de/project/showfiles.php?group_id=4124)

## Profile Replacement 

Student: Floris Bruynooghe

Weblog: [http://bruynooghe.blogspot.com/](http://bruynooghe.blogspot.com/)
...
The project is registered as pyprof on savannah.nongnu.org: [http://savannah.nongnu.org/projects/pyprof](http://savannah.nongnu.org/projects/pyprof)
...
URL: [http://udu.wiki.ubuntu.com/PythonModulePackaging](http://udu.wiki.ubuntu.com/PythonModulePackaging)

The above is a dead link. The student refers to [http://easy-deb.sourceforge.net/](http://easy-deb.sourceforge.net/)
...
URL: [http://pirate.tangentcode.com/](http://pirate.tangentcode.com/)

### Proposal 

This project didn\'t get mentioned here before. Probably because it was funded through the Perl foundation. ![:)](/wiki/europython/img/smile.png ":)")
