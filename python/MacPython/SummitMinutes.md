...
#### Do we want to unify MacPython and PyObjC? {#Do_we_want_to_unify_MacPython_and_PyObjC.3F}
...
#### What is our timeline? {#What_is_our_timeline.3F}

We would like to have a [MacPython](MacPython) distribution in summer. This distribution will consist of the new IDE, [PackMan](./PackMan.html){.nonexistent}, [BuildApplet](./BuildApplet.html){.nonexistent}, [../PyObjC](./MacPython(2f)PyObjC.html) and bug fixes for the Panther Addon distribution. For the Jaguar distribution it will also contain Python 2.3.X for whatever X is current then, for the Panther Addon distribution it will rely on Apple-installed [MacPython](MacPython).

#### Who is our audience? {#Who_is_our_audience.3F}
...
[AppleScripters](./AppleScripters.html){.nonexistent} may be an interesting audience in the future, but we have to have more infrastructure finished before we actively target them.

Another interesting audience is education. For this we definitely need the new IDE.

#### What do we want in Python 2.4? {#What_do_we_want_in_Python_2.4.3F}

- new IDE

- new packman

- [../BuildApplet](./MacPython(2f)BuildApplet.html){.nonexistent} on steroids
...
Because our first aim is the summer [MacPython](MacPython) distribution for 2.3.X we are concentrating on what is needed for that: IDE, packman, [../BuildApplet](./MacPython(2f)BuildApplet.html){.nonexistent} and icon. The others are optional (or not doable before 2.4).
...
Ronald has a [../PackMan](./MacPython(2f)PackMan.html){.nonexistent} in [../PyObjC](./MacPython(2f)PyObjC.html). Jack is going to look at it. The old [../PackMan](./MacPython(2f)PackMan.html){.nonexistent} will be dropped as soon as possible.

[BuildApplet](./BuildApplet.html){.nonexistent} will be rewritten to be a front-end to bundlebuilder. Usability to the (relative) novice is going to be important. We envision that you drop a script on it, and then get a dialog with options (build standalone, use stdio console, include icns file, include plist file, include nib, etc). Defaults for these will be picked up similar to what old [../BuildApplet](./MacPython(2f)BuildApplet.html){.nonexistent} did, i.e. if you drop `foo.py` on [../BuildApplet](./MacPython(2f)BuildApplet.html){.nonexistent} it will look for `foo.icns` and if it is found fill it in, etc.

[BuildApplet](./BuildApplet.html){.nonexistent} will not only be able to save the resulting applet, but optionally also the bundlebuilder call you would use to create it. [../BuildApplet](./MacPython(2f)BuildApplet.html){.nonexistent} should be a GUI only, if it needs functionality this will be put into bundlebuilder.

Whether [../BuildApplet](./MacPython(2f)BuildApplet.html){.nonexistent} will be a front-end to bundlebuilder or to Bobs next generation bundlebuilder remains to be seen, probably the latter. In that case it would also acquire functionality (but somewhat hidden so as not to daunt newcomers) to select which modules to include/exclude, etc.
...
On the pythonw issue: for 2.4 we should add something like MacOS.NeedWM() (in addition to the existing WMAvailable()). This would test whether WM access is available, and if not print a decent human-readable error message and exit. We would then use this in [EasyDialogs](./EasyDialogs.html){.nonexistent}, probably Tkinter, etc etc etc. Not before 2.4, though.
...
#### Who is going to work on the new IDE, and when\... {#Who_is_going_to_work_on_the_new_IDE.2C_and_when...}

- [PyOxide](./PyOxide.html){.nonexistent}?
...
Fortunately Ronald did take notes ![:-)](/wiki/europython/img/smile.png ":-)"){height="16" width="16"}

We\'ve discussed some of the alternatives:

- The [../PyOxide](./MacPython(2f)PyOxide.html){.nonexistent} license might be a problem, it is also not written in Python
...
#### Serious bugs/omissions/problems to fix: {#Serious_bugs.2Fomissions.2Fproblems_to_fix:}
...
We\'re going to investigate the installer problem, it could be that it is 10.2 only. Besides the referenced problem there is also the (null) problem if you build a [../RootDiskOnly](./MacPython(2f)RootDiskOnly.html){.nonexistent} installer. The [../PyObjC](./MacPython(2f)PyObjC.html) and Python installer builder modules are going to be merged.

bundlebuilder2 is going to be a package, so it can include [PyMacApp](./PyMacApp.html){.nonexistent}. It\'s going to have a different name (appbundle and various others flew around, Bob will pick something). It\'s also going to have other new nifty things.
...
For 2.3.X we put scripts in `/Library/Python/2.3/bin` and includes in `/Library/Python/2.3/include` through some [../PackMan](./MacPython(2f)PackMan.html){.nonexistent} magic. [../PackMan](./MacPython(2f)PackMan.html){.nonexistent} will also try to pass the include directory to later builds.
...
#### What do we need to integrate, and when? {#What_do_we_need_to_integrate.2C_and_when.3F}

- [../PyMacApp](./MacPython(2f)PyMacApp.html){.nonexistent}

  - needs different name (cf. [MacApp](./MacApp.html){.nonexistent})
...
[PyMacApp](./PyMacApp.html){.nonexistent} is goint to be renamed, and there\'s no reason to make it optional (at least, not for Panther). The plist-file dependency is not going to be optional, because of the \"explicit is better than implicit\" dictum.

Bob is going to talk to Hamish on [AppScripting](./AppScripting.html){.nonexistent}.

We are going to bug DSP until he delivers OSA.

#### Internal things {#Internal_things}
...
#### What do people still have up their sleeve? {#What_do_people_still_have_up_their_sleeve.3F}

- Bob\'s [../PackMan](./MacPython(2f)PackMan.html){.nonexistent} helper

- Jack\'s [../PackManPro](./MacPython(2f)PackManPro.html){.nonexistent} (non-existent)?

later.

#### What do we need from the general Python community? {#What_do_we_need_from_the_general_Python_community.3F}
...
- [PyArg](./PyArg.html){.nonexistent}\_Parse
