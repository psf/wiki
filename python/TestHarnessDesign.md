...
Over the next couple of hours a wrote a crude shell (in Python) for sending commands and key sequences to the phone, and reading back raw serial responses and screens back. While crude this shell does offer command editing and history (via [readline](http://docs.python.org/lib/module-readline.html){.http}) with automatically persistent history, it saves logs of all output to the phone (seperate logs for raw serial commands and for the key sequences which are embedded therein) and all input read back from the phone (again in separate streams, for raw characters read from the device and for each screenshot I take), and it allows me to insert comments into the history and log files (so I can take notes in the shell that I can correlate to specific entries in the logs).

Also my crude interactive shell uses the [optparse](http://docs.python.org/lib/module-optparse.html){.http} module to initialize my serial connection (different phones have different speeds and are connected to different ports \-\-- I have an 8-port Moxa board installed on the Linux desktop workstation). I also played a littled with [ConfigParser](http://docs.python.org/lib/module-ConfigParser.html){.http} so I could store most of the connection details and log file names in an .ini file and just call my script with a label representing which phone/port I want to play with.
...
I\'ve been looking at packages like [PExpect](http://pexpect.sourceforge.net/){.http} (*Expect* for Python), and at the [doctest](http://docs.python.org/lib/module-doctest.html){.http} and [unittest](http://docs.python.org/lib/module-unittest.html){.http} standard libraries, as well as some of the discussions around [c2.com:TestingFrameWork](http://c2.com/cgi/wiki?TestingFramework){.http} for inspiration.
...
``` {#CA-589ed48bc0ff96039e536b2c78f1e46e963f2b2d dir="ltr" lang="en"}
   1 # ...
...
This is a bit better. I can also implement methods like **`setFactoryDefaults()`{.backtick}** and **`selectMenu()`{.backtick}** and I can have the [PhoneTest](./PhoneTest.html){.nonexistent} class instantiate and contain a Phone object which abstracts some things (like the fact that accessing the menus on some models takes two keys rather than one key, and that the \"Enter\" key symbol is called \"OK\" on it, etc). (My function to send key sequences parses abstract common key notation into a dictionary of dictionaries keyed by each phone model and returning the real key symbols and sequences, it has to do things like press 2 three times for a letter \"c\" and detect if the next letter/digit is on the same button to pause for a full second to make the phone \"take\" that letter).
...
:::: {.highlight .python}
::: {.codearea dir="ltr" lang="en"}
``` {#CA-7a99ba080fce378cbf01b4be1ad2cb2d7dcdebae dir="ltr" lang="en"}
   1 # ....
   2 for i in range(99):
...
:::: {.highlight .python}
::: {.codearea dir="ltr" lang="en"}
``` {#CA-0bc988118d0c8b9668f2e8d40614a372097f68bb dir="ltr" lang="en"}
   1 # ...
   2 tst = PhoneTest(port='/dev/ttyM0', speed=9600, ...)
   3 tst.send("# a raw text command")
...
:::: {.highlight .python}
::: {.codearea dir="ltr" lang="en"}
``` {#CA-3848ac436f23c3670047eb6634972b7d814c43b9 dir="ltr" lang="en"}
   1 # ...
   2 tst = PhoneTest(port='/dev/ttyM0', speed=9600, ...)
...
:::: {.highlight .python}
::: {.codearea dir="ltr" lang="en"}
``` {#CA-770d3a092c31eb4c55700548b26f2eb3bd163b7e dir="ltr" lang="en"}
...
:::: {.highlight .python}
::: {.codearea dir="ltr" lang="en"}
``` {#CA-fc4fde0ef09cac62b9cad61b00d2fa0e8e39f6a8 dir="ltr" lang="en"}
   1 # ...
   2 try:
   3    tst.idle()
...
*I see you\'ve looked at the [unittest](http://docs.python.org/lib/module-unittest.html){.http} module that comes with the standard Python distribution. In what ways is that lacking? Have you looked at Holger Krekel\'s [py.test](http://codespeak.net/py/current/doc/test.html){.http} library?*
