...
Remember: **this is a *Wiki* \-- you can edit it simply by clicking on the *Edit Text* link at the bottom of the page**.

## python.org Webmasters {#python.org_Webmasters}

Review operation of www.python.org to make sure all current webmasters understand how it works, plus lay the groundwork for future changes.
...
## Python Scripting with Excel {#Python_Scripting_with_Excel}
...
[Alan McIntyre](mailto:alan@norfolkgraphics.com){.mailto} March 25: I have some interest in this; we have prototyped some engineering analysis tools in Excel/VBA, and it would be nice if our future prototyping could be done with Python instead of VBA (assuming I can convince the engineers to learn to use Python). ![:)](/wiki/europython/img/smile.png ":)"){height="16" width="16"}

## Packaging software for End Users {#Packaging_software_for_End_Users}

Discuss issues surrounding the packaging of applications so that end-users can install them and use them. Share techniques and secrets for building distributable files that contain python code, including (but not limited to) py2exe, distutils in general, [McMillan](./McMillan.html){.nonexistent}\'s installer, and modules that use import hooks and other magic that cause trouble with the above techniques. Focus on Windows and MacOS optional (in general it is easier to distribute software on Unix).
...
## User Interfaces in Python {#User_Interfaces_in_Python}
...
- Alternative approaches to user interfaces:
  i.  Donald A. Norman\'s ideas; ii. Jef Raskin\'s work; iii. Virtual instrumentation.

## Twisted networking framework {#Twisted_networking_framework}
...
## Toward a Python Module Repository {#Toward_a_Python_Module_Repository}
...
## Hacking Society Meeting {#Hacking_Society_Meeting}

The goal of the Hacking Society ([http://www.hackingsociety.org/](http://www.hackingsociety.org/){.http}) is to foster geek community-building through the shared experience of hacking. I propose a Hacking Society meeting on Thursday from 8pm to 11pm. More details on where will be provided via fliers at the show.

## 3D Graphics {#A3D_Graphics}
...
[Alan McIntyre](mailto:alan@norfolkgraphics.com){.mailto} March 25: I will probably be building some data visualization tools for a condition-based maintenance system in the not-so-near future (12-18 months) and would be interesting in finding out about others experiences with visualizing large data sets with Python. I have tinkered with [vtk](http://www.vtk.org/){.http} in the past but have not had time to do anything in-depth.
...
## The Python Business Forum {#The_Python_Business_Forum}

For people who apply Python in commercial environments. An introduction to what the PBF is and aims to be.

## Python Business Forum members {#Python_Business_Forum_members}

An informal discussion between members about how we can improve the PBF.

## Chandler project {#Chandler_project}

Chandler is a Python-based networked PIM currently in development at OSAF ([http://www.osafoundation.org/](http://www.osafoundation.org/){.http}). We are commited to using existing code whenever feasible, and will be pushing the boundaries of development in many areas: database, security, GUI, networking, scripting. This BOF is for anyone interested in our current plans, we can discuss architecture, libraries currently being used, plans for the future. I propose an hour-long meeting sometimes during the conference, and a dinner date on Tuesday, Wednesday or Thursday.

## Leo {#Leo}

Leo is Python-based programmer\'s editor based on outlines and literate programming techniques ([http://webpages.charter.net/edreamleo/front.html](http://webpages.charter.net/edreamleo/front.html){.http}). This BOF session will discuss where Leo is headed in the future: new collaboration features, performance enhancements, better gui\'s, other features. I propose an hour-long meeting sometimes during the conference, and dinner on Wednesday or Thursday.

## Bioinformatic/Biomedical Science uses of Python {#Bioinformatic.2FBiomedical_Science_uses_of_Python}
