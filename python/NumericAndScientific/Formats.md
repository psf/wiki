...
- [Unidata NetCDF](http://www.unidata.ucar.edu/packages/netcdf/){.http} Interfaces The netCDF datafile format stores large, uniform, data arrays efficiently and avoids byte-order problems when moving binary data between different machines. It is well-documented and looks like a good compromise between simplicity and generality.

  - \+ [NetCDF interface in ScientificPython](http://dirac.cnrs-orleans.fr/ScientificPython/){.http} - NetCDF interface that makes array variables look like [NumPy](NumPy) arrays. ([KonradHinsen](KonradHinsen))

    \+ [NetCDF interface](http://snow.cit.cornell.edu/noon/ncmodule.html){.http} - Interface to NetCDF portable data files (William Noon)

    \+ [interface](http://www.geog.ubc.ca/~kschalmNetCDF){.http} - A Numeric-Python aware NetCDF portable data file interface (Kyle Schalm)

    \+ [PyNIO](http://www.pyngl.ucar.edu/Nio.shtml){.http} - A Numeric-based Python package that allows read and/or write access to a variety of data formats (NetCDF, HDF 4, GRIB) using an interface modelled on [KonradHinsen](KonradHinsen)\'s [NetCDF interface](http://dirac.cnrs-orleans.fr/ScientificPython/){.http}.

  <!-- -->

  - \+ [PyPnetCDF](http://www.pyacts.org/pypnetcdf){.http} - A Numeric-based Python package that allows read and/or write access to NetCDF file in a parallel environment using MPI and an interface to the [PnetCDF](http://www-unix.mcs.anl.gov/parallel-netcdf/){.http} library. The object PNetCDFVariable and PNetCDFFile are very similar to [KonradHinsen](KonradHinsen)\'s definitions but PyPnetCDF implements a parallel access in a transparent and simple way to the programmer.

    \+ [netcdf4-python](http://code.google.com/p/netcdf4-python/){.http} - python/numpy interface to netCDF version 4 library. netCDF version 4 has many features not found in earlier versions of the library, such as hierarchical groups, zlib compression, multiple unlimited dimensions, and new data types. It is implemented on top of HDF5. This module implements many of the new features, and can read and write netCDF files compatibile with older versions of the library. The API is modelled after Scientific.IO.NetCDF, and should be familiar to users of that module (Jeff Whitaker).

    \+ [pupynere](http://pypi.python.org/pypi/pupynere/1.0){.http} - a PUre PYthon NEtcdf REader, and now also a Writer. Pupynere implements the NetCDF specification from scratch, written in pure Python, and only depends on Numpy. It uses the same syntax as the Scientific.IO.NetCDF module, and allows you to read and create NetCDF files.
...
- HDF5 interfaces
  - Interface to the [HDF5](http://hdf.ncsa.uiuc.edu/HDF5/){.http} format (hierachically organised datasets).
  ...
  - \+ [PyHL interface](ftp://ftp.hdfgroup.org/HDF5/contrib/hl-hdf5/README.html){.ftp} - A High Level Interface to the HDF5 File Format. (Anders Henja and Daniel B. Michelson)

    \+ [PyTables interface](http://pytables.sf.net/){.http} - HDF5 interface with full support of 64-bit data address and data indexing. (Carabos Coop. V.)

Python-specific Tools

- [TableIO](http://php.iupui.edu/~mmiller3/python/#TableIO){.http} by Mike Miller.
...
- [Scientific.IO.FortranFormat](http://dirac.cnrs-orleans.fr/ScientificPython/){.http} by [KonradHinsen](KonradHinsen).
...
- [numpyio](http://starship.python.net/~da/Travis/numpyio/){.http} by Travis Oliphant.
