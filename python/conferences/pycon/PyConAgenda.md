# PyConAgenda

```{admonition} Legacy Wiki Page
:class: note

This page was migrated from the old MoinMoin-based wiki. Information may be outdated or no longer applicable. For current documentation, see [python.org](https://www.python.org).
```

## Conference Management Board 

Put issues and their resolutions here.

## Agenda for March 12 (Wednesday) IRC meeting 

Time 4pm EST == 1pm PST == 22:00 in much of Europe.

#### Present 

Guido (chairing and taking notes here), Aahz, Brett Cannon, Tripp Lilley, Chris Armstrong, Bob Payne, Itamar.

### Catering 

Guido doesn\'t have much new info yet, the caterers will get back to him on Friday. Looks like pricing is somewhere between \$15 - \$25 depending on how fancy we want. Guido will decide this based upon projected registration.

### On-site registration process 

Need a volunteer to plan & coordinate. Tripp Lilley and Bob Payne volunteer to do this together, and will make a plan. They will post it to the mailing list this week.

### On-site network 

Tripp is volunteering to coordinate this as well; he\'s planning a site visit Friday.

### Closure 

Not much else needed immediate attention.

## Agenda for Feb 26th IRC meeting 

Members may access the [transcript](http://pydish.holdenweb.com/pycon/transcript.030226.txt) \-- *SH*

### t-shirts

#### feedback/approval of logo/shirt design 

Shop URL: [http://www.cafeshops.com/pycon1](http://www.cafeshops.com/pycon1)

Alternative color schemes: [http://www.michaelbernstein.com/logo-3-colors.jpg](http://www.michaelbernstein.com/logo-3-colors.jpg)

*Logo and color scheme approved. URL to channge to [http://www.cafeshops.com/python](http://www.cafeshops.com/python) so store can serve as general Python merch store.*

#### cafepress store, free or premium? 

Free store only allows one product of each type (one white t-shirt, one grey t-shirt, etc.). Premium store allows unlimited products and designs, and costs \$6.95/month.

*Free store under [WebMaven](./WebMaven.html) control reporting firectly to PSF and producing donations, hopefully deductible*

*All merch to have a \$2 donation added to price*

### Bag stuffing and handout preparation 

Deferred

### Web proceedings 

#### editing and publication 

#### integration of electronic slides 

### Venue staffing 

- o [SponsorRelations](./SponsorRelations.html)

  o [PressRelations](./PressRelations.html)

  o [EmergencyInstructions](./EmergencyInstructions.html)

  o [LocalKnowledge](./LocalKnowledge.html)

  o [VenueLiaison](./VenueLiaison.html)

  o [CateringCorps](./CateringCorps.html)

  o [VolunteerManagers](./VolunteerManagers.html)

  o [RoomManagers](./RoomManagers.html)

  o [AvailableVolunteers](./AvailableVolunteers.html)

  o [PyConGopher](./PyConGopher.html)

  O [RegistrationManager](./RegistrationManager.html) \[PSF interest here \...\]

  o [BadgeWriters](./BadgeWriters.html)

### Facilities 

#### On-site network hardware 

#### On-Site Computer Hardware 

There is a possibility of borrowing a number of laptops from Learning Tree International.

### Press Passes 

- Would it do any good to ask [ForeTec](./ForeTec.html)? (Definitely not! \--Guido) What about the PBF Press List o ? (Python Journal)

  - The only press coverage of IPC10?

  o Paula Ferguson (O\'Reilly)

  - Personally in favor: good relationship for [OsCon](./OsCon.html)

  O Py

  - One person, to be nominated by the publication

  o Refer enquiries to aronst?

### OtherBusiness 

#### Topics for Next Agenda 

- o [ToteBag](./ToteBag.html) (have been ordered) o Hardware and Network

There being no other business the meeting closed at 17:42 EST

------------------------------------------------------------------------

[CategoryPyCon](CategoryPyCon)
