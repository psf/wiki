...
:   [http://www.skunkweb.org](http://www.skunkweb.org){.http} (main site, but domain expired); [http://wiki.skunkweb.org](http://wiki.skunkweb.org){.http} (SkunkWeb Wiki)
...
:   your choice of [GPL](http://www.fsf.org/copyleft/gpl.html){.http} or the SkunkWeb License (a BSD license).
...
- [STML](./STML.html){.nonexistent} - a template language which uses special tags in order to control the final Web page output, and calls components written in Python or [STML](./STML.html){.nonexistent} that can either write output or return Python data. Top-level documents can also be written in Python.
