...
You might like to submit feature requests, or you may also like to contribute to the code yourself. We are also interested to hear your opinions about the possibility of adding some of our classes to [SciPy](SciPy) (perhaps in modified form). Please contact us at the [SourceForge](http://sourceforge.net/forum/?group_id=140858){.http} open discussion forum or via email.

# Features implemented {#Features_implemented}

- Efficient and state-of-the-art ODE / DAE / discrete map simulation tools (using dynamically-linked and automatically generated C code, if external compiler available) (see [wiki page](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/Generators){.http})

- Hybrid model and event-driven simulation support (see [wiki page](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/HybridSystems){.http})

- Simulations and analysis can be forced to be \"bounds safe\", e.g. for \"non-negativity preservation\" (see [wiki page](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/BoundsSafety){.http})

- Bifurcation analysis and continuation tools in-built, via [PyCont](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/PyCont){.http}

- Support for data-driven modeling (see [wiki page](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/DataDrivenModels){.http})

- Interactive command-line / script-based interface

- \"Index-free\" and context-heavy data structures, including an enhanced version of arrays (see [wiki page](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/Pointsets){.http})

- Symbolic expression utilities (including evaluation, substitution, derivatives, some simplification) (see [wiki page](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/Symbolic){.http})

- Easy to build complex models using hierarchical object-oriented data structures that contain composable model specifications (see [wiki page](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/ModelSpec){.http})

- Memory management utilities, data import & export (inc. basic SBML conversion and LaTeX markup via the [SloppyCell](./SloppyCell.html){.nonexistent} package)
...
- Seamless use with tools in [SciPy](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/SciPy){.http}, etc. through dynamic typing

- Additional toolboxes for specific applications, including biomechanical modeling, computational neuroscience, and systems biology (see [wiki page](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/ToolboxDocumentation){.http})

- Many tutorial examples and documentation available online at the [wiki](http://pydstool.sourceforge.net){.http}
...
In our design we have emphasized modularized data structures and interface design that facilitates data-driven approaches to the modeling of physical processes, and we have built upon standard numerical, scientific and graphics libraries for Python (for instance, [SciPy](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/SciPy){.http} and [Matplotlib](http://matplotlib.sourceforge.net){.http}). These, in turn, make use of well-established and efficient legacy codes for numerical integration of ODEs, and for dealing with linear algebra, optimization, and root solving (for instance, the [LAPACK](http://netlib2.cs.utk.edu/lapack/){.http} and [MINPACK](http://www.netlib.org/minpack/){.http} Fortran libraries). These legacy codes are typically interfaced using [SWIG](http://www.swig.org){.http}. The low-level languages of these codes provide the computational speed that Python itself lacks, in the places for which computation is most intensive.
...
Utilities have been added that allow the movement of data and model specifications both in and out of PyDSTool, for sharing in other software environments. As well as basic importing and exporting of numerical data as text files, this also includes more systems-level interfacing. For instance, a user can export a dynamical model's definition to a Matlab environment in which Automatic Differentiation is available for parameter sensitivity calculations (via the package ADMC++). Also, PyDSTool can be interfaced with the systems biology modeling package [SloppyCell](./SloppyCell.html){.nonexistent}, through which PyDSTool inherits access to an interface with the Systems Biology Markup Language (SBML) for model specification, and the LaTeX mathematical markup language. Further interfaces to packages are in active development, such as to the original [DsTool](http://www.cam.cornell.edu/~rclewley/cgi-bin/moin.cgi/DsTool){.http} and to other simulation tools such as [XPP](http://www.math.pitt.edu/~bard/xpp/xpp.html){.http} or [NEURON](http://neuron.duke.edu/){.http}.
:::
