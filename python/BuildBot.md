...
**[Here are the current BuildBot results](http://buildbot.python.org/all/#/console/){.http}**.

We need to add more platforms (and compilers!) to the BuildBot farm. If you have one of these machines you are willing to run BuildBot on, please contact [python-buildbots@python.org](mailto:python-buildbots@python.org){.mailto}:
...
To add a new buildbot or change the configuration of your existing one, send a pull request to the build master config found at [https://github.com/python/buildmaster-config/blob/master/master/master.cfg](https://github.com/python/buildmaster-config/blob/master/master/master.cfg){.https}.

## Installing a buildbot worker {#Installing_a_buildbot_worker}

You need to install a recent version of [Buildbot](http://buildbot.net/){.http}. If your OS has a packaging system (e.g. under Linux), it is probably available from the standard repositories. Otherwise, you\'ll have to build it from source (in which case you\'ll have to install Python and Twisted first).
...
You\'ll need to get someone to create the workername/workerpasswd on buildbot.python.org before doing this. Ask on [python-buildbots@python.org](mailto:python-buildbots@python.org){.mailto} once you\'re ready.
...
Once the buildbot is running, don\'t forget to monitor the [build results](http://www.python.org/dev/buildbot/){.http} and solve any setup issues causing test failures.
...
.. \[2\]: [http://mail.python.org/pipermail/python-dev/2012-March/117978.html](http://mail.python.org/pipermail/python-dev/2012-March/117978.html){.http}

## Buildbot Workers and Security {#Buildbot_Workers_and_Security}
