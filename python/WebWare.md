...
:   [http://www.webwareforpython.org](http://www.webwareforpython.org){.http}
...
:   [Old-style Python licence](http://www.webwareforpython.org/Docs/Copyright.html){.http} - Webware is open source.
...
The need to run the [WebKit](WebKit) component\'s application server process continually excludes Webware from certain hosting environments. It is possible to use a [OneShot](./OneShot.html){.nonexistent} CGI-based mechanism to launch the application server for each request, but performance is very poor.
...
- Kid - Webware supports Kid templates via the [KidKit](./KidKit.html){.nonexistent} plug-in.
