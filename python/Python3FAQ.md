...
# What\'s Python 3? {#What.27s_Python_3.3F}
...
# What\'s Python 3000? {#What.27s_Python_3000.3F}
...
# What changed in Python 3.0? {#What_changed_in_Python_3.0.3F}

Read [What\'s New in Python 3.0](http://docs.python.org/3.0/whatsnew/3.0.html){.http}, which lists the changes between Python 2.6+ and Python 3.0.

# What will happen to the Python 2.x versions? {#What_will_happen_to_the_Python_2.x_versions.3F}
...
The penultimate Python 2.x release, 2.7.17, is [slated to be released](https://www.python.org/dev/peps/pep-0373/#id4){.https} in October of 2019. [Release Candidate 1](https://www.python.org/downloads/release/python-2717rc1/){.https} was made available on 9 October 2019. There will be some flexibility to fix any bugs or issues specifically introduced by the 2.7.17 release in January and February of 2020.
...
# How should I port existing Python 2.x code to Python 3.x? {#How_should_I_port_existing_Python_2.x_code_to_Python_3.x.3F}
...
Consult [the 2to3 documentation](http://docs.python.org/3/library/2to3.html){.http} for more information about the conversion tool, and [PortingPythonToPy3k](PortingPythonToPy3k) for more general suggestions.
...
[In this weblog posting](http://www.artima.com/weblogs/viewpost.jsp?thread=227041){.http}, Guido van Rossum strongly suggests not changing your APIs when porting to 3.0, because API changes will make it difficult for users to tell whether test suite failures are due to the 2.6+-\>3.0 transition or to the API changes.

[Early2to3Migrations](Early2to3Migrations) has links to some early attempts at porting packages to 3.x.

# How should I port existing C extensions to Python 3.x? {#How_should_I_port_existing_C_extensions_to_Python_3.x.3F}

Benjamin Peterson\'s [Porting Extension Modules to 3.0](http://docs.python.org/howto/cporting.html){.http} describes some of the changes to Python\'s C API, and gives some tips for writing code that will compile with both 2.x and 3.0. The [draft in this wiki](PortingExtensionModulesToPy3k) may be more complete.

# How do I find out what packages have been ported to 3.x? {#How_do_I_find_out_what_packages_have_been_ported_to_3.x.3F}

Packages listed in the [Python Package Index](http://pypi.python.org){.http} can indicate whether they support Python 3.x. You can view [a list of 3.x-compatible packages](http://pypi.python.org/pypi?:action=browse&c=533){.http} by searching for the `"Programming Language :: Python :: 3"`{.backtick} classifier in the package index.

# My new release supports Python 3.x. How do I indicate that? {#My_new_release_supports_Python_3.x.__How_do_I_indicate_that.3F}

If you\'re using your `setup.py`{.backtick} file to upload a package to the [Python Package Index](http://pypi.python.org){.http}, include `"Programming Language :: Python :: 3"`{.backtick} as one of the classifiers.

# I want to start learning/teaching Python. What version should I use? {#I_want_to_start_learning.2Fteaching_Python.__What_version_should_I_use.3F}
