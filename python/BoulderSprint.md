...
For more information about sprints in general, read the [SprintIntroduction](SprintIntroduction) page. [SteveHolden](SteveHolden) has a useful [article](http://www.onlamp.com/pub/a/python/2006/10/19/running-a-sprint.html){.http} in the [OnLamp Python DevCenter](http://www.onlamp.com/python/){.http} on how to run a sprint.
...
## TurboGears World-Wide Sprint: Jan 12-13, 2007 {#TurboGears_World-Wide_Sprint:_Jan_12-13.2C_2007}

## Django World-Wide Sprint: Dec 1, 2007 {#Django_World-Wide_Sprint:_Dec_1.2C_2007}

## Django World-Wide Sprint: Sept 14-15, 2007 {#Django_World-Wide_Sprint:_Sept_14-15.2C_2007}

## Jython Sprint: Aug 4, 2007 {#Jython_Sprint:_Aug_4.2C_2007}
...
### IPython on Jython {#IPython_on_Jython}
...
- Late-breaking Homework
  1.  Grab the source for the sprint branch through svn: svn co [https://jython.svn.sourceforge.net/svnroot/jython/branches/august-boulder-sprint](https://jython.svn.sourceforge.net/svnroot/jython/branches/august-boulder-sprint){.https} jython

### Jython 2.5 {#Jython_2.5}
...
## IPython1 Sprint {#IPython1_Sprint}
...
- Visit [http://ipython.scipy.org/moin/IPython1](http://ipython.scipy.org/moin/IPython1){.http}

- Thoroughly read the wiki pages relating to IPython1

- Check out the \"saw\" branch (not \"chainsaw\") as described at [http://ipython.scipy.org/moin/Developer_Zone](http://ipython.scipy.org/moin/Developer_Zone){.http}
...
## Jython Sprints {#Jython_Sprints}

- January 6, 2007 (Saturday), 9 AM - 6 PM
- February 3, 2007 (Saturday), 9 AM - 6 PM

Update Jython. [Brian Zimmer\'s proposal](http://wiki.python.org/jython/MovingJythonForward){.http} is an useful outline of what needs to be done technically. Perhaps even more importantly, there are some key engineering challenges to be addressed to ensure overall project success. Because of the scope of the work, we are looking at this as preliminary to [a 4-day sprint at PyCon](http://us.pycon.org/TX2007/Sprints){.http}. More here at [JythonSprint](JythonSprint).
...
## Django-Oracle Project {#Django-Oracle_Project}
...
The bug report stipulating Oracle support by Django version 1.0 is here: [http://code.djangoproject.com/ticket/1990](http://code.djangoproject.com/ticket/1990){.http}

## Preliminary Tasks {#Preliminary_Tasks}
...
- Use ROWNUM, not LIMIT 1. Oracle guru Tom Kyte addresses this in a recent [article](http://www.oracle.com/technology/oramag/oracle/06-sep/o56asktom.html){.http} in Oracle magazine. It boils down to using a doubly nested subquery.
...
- (Done) Primary key \"id\" column ([AutoField](./AutoField.html){.nonexistent}) is NUMBER(38) in creation.py. Does it need to be that large? No it doesn\'t.
...
- (Done\--as is) [FormatStylePlaceholderCursor](./FormatStylePlaceholderCursor.html){.nonexistent} object: is it needed? We thought cx_Oracle already did named params\...
...
- (Done) [SlugField](./SlugField.html){.nonexistent}, [PhoneNumberField](./PhoneNumberField.html){.nonexistent}, etc., use VARCHAR and CHAR, but all should use VARCHAR2 consistently.

- [TimeField](./TimeField.html){.nonexistent} should use \"INTERVAL DAY TO SECOND\" datatype

- (Done) [NullBooleanField](./NullBooleanField.html){.nonexistent} should be NUMBER(1) like [BooleanField](./BooleanField.html){.nonexistent}, not NUMBER(38)

- (Done) Don\'t use Oracle\'s non-native Integer and [SmallInteger](./SmallInteger.html){.nonexistent} types, which result in a NUMBER(38).

- [XmlField](./XmlField.html){.nonexistent} unimplemented, although Oracle now has native support for this.
...
## Post Sprint Party {#Post_Sprint_Party}

Some of us are planning to go to the [Cuban Connection fundraiser](http://www.bouldercuba.org/){.http}, 6 PM - 1 AM, St. Julien Hotel (our favorite meeting place for [FrontRangePythoneers](FrontRangePythoneers)). In addition to raising money for a good cause, this will be a chance to stretch our weary coding muscles while dancing salsa to Quemando and Havana NRG.

## Photo Stream {#Photo_Stream}

- Nov 4, 2006 [Photos](http://www.flickr.com/search/?w=all&q=bouldersprint){.http}
:::
