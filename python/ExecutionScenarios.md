...
- child exception propagation if executing Python script ([http://docs.python.org/2/library/subprocess.html#exceptions](http://docs.python.org/2/library/subprocess.html#exceptions){.http})

- check_call() or check_output() return exception on non-zero exit code (other functions do not)

#### Fire and Forget {#Fire_and_Forget}

##### Crossplatform shell automation {#Crossplatform_shell_automation}
...
#### Libraries on Execution {#Libraries_on_Execution}

- [http://amoffat.github.com/sh/](http://amoffat.github.com/sh/){.http} - avoids suprocess altogether, linux-only for now

#### Notes about bad design in subprocess {#Notes_about_bad_design_in_subprocess}
