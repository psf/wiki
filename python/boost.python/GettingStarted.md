...
The [BoostPython](BoostPython) Library binds C++ and Python in a mostly-seamless fashion. It is just one member of the boost C++ library collection at [http://www.boost.org](http://www.boost.org){.http}.
...
The Python [C++-sig](http://www.python.org/sigs/current/cplusplus-sig/){.http} serves as a mailing list for users of the library. Documentation for the current release is available at [http://www.boost.org/libs/python/](http://www.boost.org/libs/python/){.http}. Development documentation, which is usually more up-to-date, is available through the [Boost CVSWeb](http://cvs.sourceforge.net/cgi-bin/viewcvs.cgi/*checkout*/boost/boost/libs/python/doc/index.html){.http} interface.

### Relationship to the Python C API {#Relationship_to_the_Python_C_API}
...
### If you need to get to the underlying Python/C API {#If_you_need_to_get_to_the_underlying_Python.2FC_API}
...
In C++, the Python/C API represents Python objects by [PyObject](./PyObject.html){.nonexistent} pointers. In Boost::Python, these are wrapped by instances of the boost::python::object class.

If you need the underlying [PyObject](./PyObject.html){.nonexistent} of any boost::python::object, you can get it via the ptr() method of boost::python::object, which returns a [PyObject](./PyObject.html){.nonexistent}\*. You can then use it in Python/C API calls. For example, to test if a boost::python::object called boostObj has an attribute called \"myAttributeName\", you can do:
