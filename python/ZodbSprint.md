...
- Multi-database support, [http://zope.org/Wikis/ZODB/MultiDatabases](http://zope.org/Wikis/ZODB/MultiDatabases){.http}. PROGRESS: A simpler of version of this proposal was implemented. Still needed: work on Zope to use this, and to stop monkey-patching ZODB internals.
...
- Blob support. PROGRESS: Enormous progress on this was made, primarily by Christian Theune and Chris [McDonough](./McDonough.html){.nonexistent}. See the zodb-dev mail list for some detailed status reports and timings. The code is in ZODB/branches/ctheune-blobsupport/.
...
Note that to be able to make code contributions, you must be a Zope contributor: [http://dev.zope.org/Subversion](http://dev.zope.org/Subversion){.http} Note that you don\'t have to be a Zope user to be a Zope contributor. ![:)](/wiki/europython/img/smile.png ":)"){height="16" width="16"} You don\'t have to be a Zope contributor either to participate in working out (and writing up) proposals for things ZODB could/should be doing better.
...
- Chris [McDonough](./McDonough.html){.nonexistent} (Sat, Sun)
...
We seem to have a nascent idea about working on ZODB \"blob\" suppport for at least some period of time during this sprint\... for details, see [http://www.plope.com/Members/chrism/zodb_blob_proposal](http://www.plope.com/Members/chrism/zodb_blob_proposal){.http}
...
- \- Make ZEO server write a pidfile so we can rotate its logs by sending an appropriate
  ...
  - making Tim\'s life a nightmare. But Tim thinks this is
    ...
    pretty hopeless ![;-)](/wiki/europython/img/smile4.png ";-)"){height="16" width="16"}
