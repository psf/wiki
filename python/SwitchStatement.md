...
Many languages have a SwitchStatement Aka [CaseStatement](./CaseStatement.html){.nonexistent}. Generally, these languages don\'t have convenient mapping abstractions built in, so a [CaseStatement](./CaseStatement.html){.nonexistent} provides a syntatic sugar (as opposed to an abstraction) to help write constructs like
