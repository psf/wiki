...
# Tasks for the sprint {#Tasks_for_the_sprint}
...
- PEP 376 compatible installs
  ...
  - Decided that a proof-of-concept implementation of PEP 376 installs would be cleaner and clearer if implemented directly in distutils (in a standalone branch for now), rather than layering it into distribute 0.7 on top of current distutils. Ideally this distutils branch could be installed for testing just like the distutils nightly builds at [http://nightly.ziade.org](http://nightly.ziade.org){.http} on all recent Python versions.

  - Converted distutils into standalone hg repo at [http://bitbucket.org/carljm/python-distutils/](http://bitbucket.org/carljm/python-distutils/){.http}
