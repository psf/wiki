...
# The Python intermediate\'s guide: Python packages confusion reducer {#The_Python_intermediate.27s_guide:_Python_packages_confusion_reducer}
...
- [Python2 vs. Python3 differences](http://docs.python.org/3/whatsnew/3.0.html){.http}
...
1.  [The Python intermediate\'s guide: Python packages confusion reducer](#The_Python_intermediate.27s_guide:_Python_packages_confusion_reducer)
    1.  [Python3 versus Python2](#Python3_versus_Python2)
    2.  [Package installation](#Package_installation)
    3.  [Multiple isolated Python environments](#Multiple_isolated_Python_environments)
    4.  [Creating a package installer or application installer](#Creating_a_package_installer_or_application_installer)
    5.  [GUI programming](#GUI_programming)
    6.  [Web development](#Web_development)
    7.  [Object-relational mapping](#Object-relational_mapping)
:::

------------------------------------------------------------------------

## Python3 versus Python2 {#Python3_versus_Python2}

You should use Python 3 going forward. As of January 2020 [Python 2 will be in EOL (End Of Life) status](https://www.python.org/dev/peps/pep-0373/){.https} and receive no further official support. After that date, there will be no further updates nor bugfixes. Since this end-of-life date has been planned for nearly a decade (the first end-of-life date was slated to happen in 2014, and was pushed back to 2020), and nearly all popular libraries [have already ported their code](https://python3statement.org/){.https}, Python 2.x is well on its way to obsolescence. As such, we can only recommend learning and using Python 3.

## Package installation {#Package_installation}

- [PyPI](http://pypi.python.org){.http} is for Python what [CPAN](http://www.cpan.org/){.http} is for Perl and what [CTAN](http://ctan.org/){.http} is for TeX: an almost comprehensive catalog of the freely available reusable components.

- For downloading and installing PyPI packages and keeping a set of such installations up to date, [pip](http://www.pip-installer.org){.http} is far more powerful than `easy_install` and should almost always be preferred.

## Multiple isolated Python environments {#Multiple_isolated_Python_environments}
...
- [venv](http://docs.python.org/3/library/venv.html){.http} implements virtual environments for Python3. It is a core package and technically subtly superior to virtualenv.

You should use a virtual environment for any team project and for any package you intend to distribute.

## Creating a package installer or application installer {#Creating_a_package_installer_or_application_installer}
...
See the documentation of [distutils](http://docs.python.org/3/distutils/){.http} from the standard library. Distutils will not only make `setup.py` easy to write, it will also provide it with functionality for *creating* the distributable package file in the first place.

In contrast to some other platforms it is common in Python that neither a reusable package nor a distributable application come packaged with all the additional reusable packages they depend on. Rather, the dependencies are only *declared* in `setup.py` and an installer program such as [pip](http://www.pip-installer.org){.http} will download and install additional packages (that are yet missing or the version of which is inappropriate) automatically.
...
So what should you use? First, be aware that many packages on PyPI will `import setuptools}, so you need to have {{{setuptools` or `distribute` installed in any case. Second, for an up-to-date answer for your own development, see the [summary on stackoverflow](http://stackoverflow.com/questions/6344076/differences-between-distribute-distutils-setuptools-and-distutils2){.http} and the [Python Packaging User Guide recommendation](https://python-packaging-user-guide.readthedocs.org){.https}.

## GUI programming {#GUI_programming}

There is a surprising range of toolkits for building desktop GUIs with Python. One of them, [tkinter](http://docs.python.org/3/library/tkinter.html){.http}, is a core package consisting of Python bindings for TCL/TK\'s tk plus additional widgets (in [tkinter.ttk](http://docs.python.org/3/library/tkinter.ttk.html){.http} and [tkinter.tix](http://docs.python.org/3/library/tkinter.tix.html){.http}). tkinter is platform-independent and easy to program for simple GUIs.

For larger applications, you might prefer [a more extensive GUI toolkit](GuiProgramming).

## Web development {#Web_development}

There is a surprising range of frameworks for building web applications with Python. Several of them are quite good. They follow fairly different objectives (such as being slim vs. extensive or regarding the development style they imply). Among these, [Django](https://www.djangoproject.com/){.https} has the largest and most active community by far.

## Object-relational mapping {#Object-relational_mapping}

There are at least two mature, powerful, and widely used solutions for O/R mapping:

- [sqlalchemy](http://www.sqlalchemy.org/){.http}

- [Django](https://www.djangoproject.com/){.https}\'s persistence framework
