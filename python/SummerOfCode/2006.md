...
::: {#SummerOfCodePdb.content dir="ltr" lang="en"}
# Improving the Python debugger {#SummerOfCodePdb.Improving_the_Python_debugger}
...
## Debugging communication techniques and ideas {#SummerOfCodePdb.Debugging_communication_techniques_and_ideas}
...
# Base multidimensional array type for Python core {#Base_multidimensional_array_type_for_Python_core}

Student: [KarolLangner](KarolLangner)

Mentor: Travis E. Oliphant

Repository: [http://svn.scipy.org/svn/PEP/](http://svn.scipy.org/svn/PEP/){.http}

More information: [http://scipy.org/BaseArray](http://scipy.org/BaseArray){.http}

The goal is to prepare a simple, generic multidimensional array interface that can be readily placed in the Python core as a new built-in base type (called, for instance, \'dimarray\'), and included in a future Python distribution (possibly 2.6?). This new base type will have the same C-structure as the current array implementation in [numpy](http://scipy.org/NumPy){.http} and will be based on an interface recently formulated by Travis Oliphant within a Python Enhancement Proposal ([http://svn.scipy.org/svn/PEP/](http://svn.scipy.org/svn/PEP/){.http}). After preparing a \'ready to insert\' version of the array interface, it will be applied to numpy and several other packages that work with multidimensional data, and possibly modified in order to work out an optimal scope.

# Logging Usage in the Standard Library (PEP 337) {#Logging_Usage_in_the_Standard_Library_.28PEP_337.29}

Student: Jackilyn Hoxworth

Mentor: James Joseph Jewett

Repository: [http://svn.python.org/projects/python/branches/hoxworth-stdlib_logging-soc/](http://svn.python.org/projects/python/branches/hoxworth-stdlib_logging-soc/){.http}

[Project Blog](http://www.jackilyn.com/category/google-soc/){.http}
...
# Decimal module in C. {#Decimal_module_in_C.}

Student: Mateusz Rukowicz

Mentor: Facundo Batista

Repository: [http://svn.python.org/view/sandbox/trunk/decimal-c/](http://svn.python.org/view/sandbox/trunk/decimal-c/){.http}
...
# Rewrite of the zipfile module. {#Rewrite_of_the_zipfile_module.}

Student: Nilton Volpato

Mentor: Ilya Etingof

Repository: [https://svn.sourceforge.net/svnroot/ziparchive/ziparchive/trunk](https://svn.sourceforge.net/svnroot/ziparchive/ziparchive/trunk){.https}
...
# Ajax in python based on PyPy\'s JavaScript backend {#Ajax_in_python_based_on_PyPy.27s_JavaScript_backend}

Student: Maciej Fijalkowski

Mentor: Eric van Riet Paap

Repository: [http://codespeak.net/svn/pypy/dist/pypy/translator/js/](http://codespeak.net/svn/pypy/dist/pypy/translator/js/){.http}

The main target of the project is to produce a framework which takes advantage of [PyPy](PyPy)\'s [JavaScript](./JavaScript.html){.nonexistent} backend and to make it easy to write an AJAX based framework written entirely in Python.

The aim is to produce a framework which allows Python applications to be translated into an AJAX based application split into a client part ([JavaScript](./JavaScript.html){.nonexistent} running on browser) and a server part (python running on web server). Communication between client and server is translated from Python method calls into JSON request and response.

# Complete gencli, the PyPy CLI backend {#Complete_gencli.2C_the_PyPy_CLI_backend}

Student: Antonio Cuni

Mentor: Armin Rigo

Repository: [http://codespeak.net/svn/pypy/dist/pypy/translator/cli/](http://codespeak.net/svn/pypy/dist/pypy/translator/cli/){.http}

gencli is the [PyPy](PyPy) backend targeting the Common Language Infrastructure (CLI) virtual machine, i.e. .NET.

Once gencli will be complete it will be possible to translate the Standard Interpreter in order to produce a .NET executable of the Python interpreter (we could call it [PyPy](PyPy).NET).

The result will be in some way similar to the existing [IronPython](IronPython), with the difference that [IronPython](IronPython) is a compiler (though most of the job is done by the runtime environment), while [PyPy](PyPy).NET will be an interpreter.

# PyPy Proposal - Write and port modules from CPython with ctypes {#PyPy_Proposal_-_Write_and_port_modules_from_CPython_with_ctypes}
...
[http://codespeak.net/svn/user/rhymes/modules/](http://codespeak.net/svn/user/rhymes/modules/){.http} [http://codespeak.net/svn/pypy/dist/pypy/module/](http://codespeak.net/svn/pypy/dist/pypy/module/){.http}

I\'d like to make [PyPy](PyPy) ready for encrypted communications adding SSL support in the ongoing and in-development socket module. The other modules I\'ll port are: bz2, fcntl, mmap and time (very fundamental). If there\'s time I\'d like to start working on \'os\' and \'select\' modules also.

# Neural Nets in SciPy {#Neural_Nets_in_SciPy}

Student: Frederic Mailhot

Mentor: Robert Kern

Repository: [http://svn.scipy.org/svn/scipy/trunk/Lib/sandbox/ann/](http://svn.scipy.org/svn/scipy/trunk/Lib/sandbox/ann/){.http}

The goal of this project is to extend [SciPy](SciPy)\'s functionality by adding modules for the design, training and use of a variety of neural network architectures, including standard feedforward and recurrent networks, among others. As a guide I intend to work from the modules in Matlab\'s Netlab toolkit, as well as from my own experience implementing recurrent networks.

# Enhancements to mathtext (part of matplotlib) - a Python package for typesetting {#Enhancements_to_mathtext_.28part_of_matplotlib.29_-_a_Python_package_for_typesetting}

Student: Edin Salković

Mentor: John D Hunter

Repository: [http://svn.sourceforge.net/viewcvs.cgi/matplotlib/trunk/matplotlib/lib/matplotlib/](http://svn.sourceforge.net/viewcvs.cgi/matplotlib/trunk/matplotlib/lib/matplotlib/){.http}
...
# Soya3D Collision API : Improving ODE integration in the core {#Soya3D_Collision_API_:_Improving_ODE_integration_in_the_core}
...
[Soya](http://www.soya3d.org/){.http} lacks some very useful tools such as a collision API or a properly integrated physics engine.

I am applying to Summer of Code 2006 in order to fix these deficiencies by improving support for the [Open Dynamics Engine](http://www.ode.org/){.http}, which is barely working and too low-level in the last release of Soya, and provide a similar collision detection system to all objects whether ODE is in use or not.

# PySoy & Cal3D exporting/importing tools for Blender {#PySoy_.26_Cal3D_exporting.2Fimporting_tools_for_Blender}

Student: Palle Raabjerg

Mentor: Buddha Michael Dylan Buck

Repository: [http://svn.pysoy.org/trunk/blender-soy](http://svn.pysoy.org/trunk/blender-soy){.http}

Browse Source: [http://www.pysoy.org/browser/trunk/blender-soy](http://www.pysoy.org/browser/trunk/blender-soy){.http}

Currently, no [PySoy](http://www.pysoy.org/){.http} importers exist for Blender. This means you cannot import any Soy or Cal3D models into Blender. This can be inconvenient for both the game developer, as he needs to publish the .blend files to allow people to modify models, and for a gamer community surrounding a project, as they can\'t modify models unless the developer publishes the .blend files. (Note: This might be a perfectly okay and even preferable situation for some proprietary game developers, but [PySoy](PySoy) is built on a philosophy of openness and freedom, making this a more important feature.)

The [PySoy](PySoy) exporter tools basically consists of the blender2soya and blender2cal3d scripts. blender2soya is currently used for static (non-animated) models, and blender2cal3d is used for exporting animated skeletal models to the Cal3D format, which can also be used with [PySoy](PySoy). These two scripts lack many features, however, and still contain known flaws.
...
If I am assigned to this project, I will create [PySoy](http://www.pysoy.org/){.http} and Cal3D importers for Blender, and I will work on both exporter scripts

*Due to an inconviently-timed project fork, this project has been moved to [PySoy](PySoy) (was formerly for Soya3D). Beside the name of the project, all aspects remain the same and the code will be available for the Soya3d project and any other project which needs it.*

# Pygame on ctypes {#Pygame_on_ctypes}

Student: Alex Holkner

Mentor: Pete Shinners

Repository: [http://www.pygame.org/ctypes](http://www.pygame.org/ctypes){.http}

My project proposal is to rewrite Pygame to use ctypes. The current implementation is written as a C module that links to SDL. The proposed addition of ctypes to Python 2.5 is a great catalyst for using it to wrap SDL and reimplementing Pygame in pure Python. This would allow developers to extend Pygame with much more ease than is currently possible, and to make use of SDL features not exported by Pygame, and to give [PyPy](PyPy) development another library.

# SQLAlchemy Schema Migration {#SQLAlchemy_Schema_Migration}

Student: Evan Pierce Rosson

Mentor: [Jonathan LaCour](http://cleverdevil.org){.http}

Repository: [http://trac.erosson.com/migrate](http://trac.erosson.com/migrate){.http}
...
# Drop-in WSGI support for Commodity Hosting {#Drop-in_WSGI_support_for_Commodity_Hosting}
...
[Project Home](http://code.google.com/p/grail-soc/){.http}
...
# PyDev - Python for Eclipse {#PyDev_-_Python_for_Eclipse}

Student: [Sean Handley](http://www.seanhandley.com){.http}

Mentor: [Fabio Zadrozny](http://www.blogger.com/profile/4798968){.http}

Repository: [http://pydev.cvs.sourceforge.net/pydev/](http://pydev.cvs.sourceforge.net/pydev/){.http}

Project Blog: [http://www.planet-soc.com/?q=blog/114](http://www.planet-soc.com/?q=blog/114){.http}

My objectives for [PyDev](PyDev) are to implement as many of the most popular requested features as I can, bringing the power of Python development in Eclipse up to the same standards as Java. Specifically, I will be adding new features to the debugger and code editor.

# Coder: An extensible web-based programming tutor {#Coder:_An_extensible_web-based_programming_tutor}

Student: Johannes Woolard

Mentor: André Roberge

Repository: [http://svn.crunchy.python-hosting.com/branches/jwsoc](http://svn.crunchy.python-hosting.com/branches/jwsoc){.http}
...
# Improving Mailman\'s User Experience {#Improving_Mailman.27s_User_Experience}

Student: Ethan Fremen

Mentor: Barry Warsaw

Repository: [https://svn.sourceforge.net/svnroot/mailman/branches/soc2006-webui](https://svn.sourceforge.net/svnroot/mailman/branches/soc2006-webui){.https}
...
# Memory Optimization in Shed Skin {#Memory_Optimization_in_Shed_Skin}

Student: Zheng Siyao

Mentor: Mark Dufour

Repository: [http://shedskin.cvs.sourceforge.net/shedskin/shedskin/memopt/](http://shedskin.cvs.sourceforge.net/shedskin/shedskin/memopt/){.http}

Shed Skin is an optimizing Python-to-C++ compiler, that accepts pure, but implicitly statically typed Python programs. It already converts many \'smallish\' programs, such as a ray tracer, chess player, several sudoku solvers, etc., with an average relative speedup of about 12 over Psyco, and about 45 over CPython. However, Shed Skin is still at an early release, and there are several integration issues that need to be solved. For example, programs can currently only use several supported standard library functions. See [http://mark.dufour.googlepages.com](http://mark.dufour.googlepages.com){.http} for more details.
...
# YAML parser and emitter {#YAML_parser_and_emitter}

Student: Kyrylo Simonov

Mentor: Clark C. Evans

Homepage: [http://pyyaml.org/wiki/LibYAML](http://pyyaml.org/wiki/LibYAML){.http}

Repository: [http://svn.pyyaml.org/libyaml/trunk/](http://svn.pyyaml.org/libyaml/trunk/){.http}

Timeline: [http://pyyaml.org/timeline](http://pyyaml.org/timeline){.http}
...
# Web-based administration interface for DrProject {#Web-based_administration_interface_for_DrProject}

Student: Gregory Lapouchnian

Mentor: Greg Wilson

Trac / Repository: [http://pyre.third-bit.com/drproject/drproject-dev/](http://pyre.third-bit.com/drproject/drproject-dev/){.http}

Currently the administration of a [DrProject](./DrProject.html){.nonexistent} installation has to be done through a command line script. This works well in certain situations where a professor is able (or willing) to write scripts to make use [DrProject](./DrProject.html){.nonexistent}\'s API in order to execute batch operations. However a web-based administration interface would greatly simplify course management by providing a central location for common tasks such as removing a student from a course or moving them from one project to another. A web-based console will also make it possible to present to the instructors some general statistics for the groups and students in their course in a convenient format.

# Cheesecake enhancements and its integration with PyPI. {#Cheesecake_enhancements_and_its_integration_with_PyPI.}

*Student*: [Michał Kwiatkowski](http://joker.linuxstuff.pl/){.http}

*Mentor*: [Grig Gheorghiu](http://agiletesting.blogspot.com){.http}

Repository: [http://pycheesecake.org/browser](http://pycheesecake.org/browser){.http}

*Project timeline and ideas*: [Cheesecake](http://pycheesecake.org/wiki/SummerOfCode06){.http} - feel free to add your comments about the project there!

[Cheesecake](http://pycheesecake.org){.http} is an application designed to evaluate and estimate the overall quality (or so called \'kwalitee\') of a given software package written in Python. It emphasizes a need for well-written documentation and unit tests, encouraging good programming practices and penalizing sloppy design and careless distribution. Using Cheesecake to check your code gives you confidence that your software doesn\'t merely run, but is usable and easy to test and modify as well.

Because Python is very easy to learn and use there exists a vast variety of software written in it, most of which was scattered until [PyPI](http://python.org/pypi){.http} was created. Now, when new packages are being indexed on Cheese Shop every day, an effort can be made to spread the spirit of good software design and code reuse among the Python community. This can be achieved by combining the power of Cheesecake and Cheese Shop. Everytime a new version of a package would be uploaded to Cheese Shop, its cheesecake index will be calculated and published on web. Having a way to measure a quality of a package with accordance to other existing packages will be of invaluable help for all developers. It will promote well built packages and in the long run raise the overall quality of Python software.

Adding Cheesecake functionality to PyPI has been already [mentioned](http://mail.python.org/pipermail/catalog-sig/2006-April/000825.html){.http} by Phillip J. Eby on the catalog-sig mailing list. Together with Cheesecake maintainer Grig Gheorghiu we\'ve discussed modifications needed to be done to Cheesecake code to be reliable enough so it could be incorporated into PyPI service. A [working copy of our ideas](http://pycheesecake.org/wiki/SummerOfCode06){.http} is accessible on the project wiki. It includes enhancing Cheesecake code scoring techniques to take into account unit tests of a package, running tests in secure environment, extending supported archive formats and fixing all known bugs. Development of Cheesecake will adhere to best practices such as unit testing, continuous integration (via buildbot), pylint verification, etc.
...
In addition to ease the creation of plugins, I would make bindings to readily enable the use of wxPython, PyGTK, [PyGame](PyGame) and Soya3D. These bindings would enable those libraries to handle the in-browser window and associated events.

# Support Vector Machines for SciPy {#Support_Vector_Machines_for_SciPy}

Student: [Albert Strasheim](http://students.ee.sun.ac.za/~albert/){.http}

Mentor: Dave Kammeyer

Repository: [http://pycheesecake.org/browser](http://pycheesecake.org/browser){.http}

Integrating libsvm (and possibly other SVM libraries) with [SciPy](http://www.scipy.org/){.http} using ctypes and providing various tools for manipulating datasets for use with SVMs.

[Project homepage](http://students.ee.sun.ac.za/~albert/pylibsvm/){.http}
::::
