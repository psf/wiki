...
# PEP 393 --- Flexible String Representation {#PEP_393_.2BIBQ_Flexible_String_Representation}

This document serves as the documentation for the initial implementation of [PEP 393](http://www.python.org/dev/peps/pep-0393/){.http} during the Google Summer of Code 2011. The project\'s repository is hosted on [bitbucket in the pep-393 branch](https://bitbucket.org/t0rsten/pep-393){.https}.
...
Please see the [PEP document](http://www.python.org/dev/peps/pep-0393/){.http} for details on the rationale and the proposed approach. The [announcement email on python-dev](http://mail.python.org/pipermail/python-dev/2011-January/thread.html#107641){.http} also gives some more context information.
...
## Changed Paradigms {#Changed_Paradigms}
...
## New Functions and Macros {#New_Functions_and_Macros}

New functions are documented in the source code ([diff of the unicodeobject.h header](https://bitbucket.org/t0rsten/pep-393/diff/Include/unicodeobject.h?diff2=pep-393&diff1=default){.https}), but as a overview here are the most important new functions and macros:
...
## Removed Optimizations and Incompatible Changes {#Removed_Optimizations_and_Incompatible_Changes}
...
## Performance Measurements {#Performance_Measurements}
...
Additionally, there exists a set of micro benchmarks to test the behaviour of the implementation in a separate [bitbucket repository](https://bitbucket.org/t0rsten/pep-393-bench){.https}.
