...
These are the Google \"Summer of Code\" projects involving Python that were funded in 2005.
...
# Python Implementation of the Data Access Protocol {#Python_Implementation_of_the_Data_Access_Protocol}

Student: Roberto Antonio Ferreira De Almeida

Mentor: Paul DuBois.

Web page: [OPeNDAP/DODS module for Python](http://opendap.oceanografia.org/){.http}
...
# Bitten: A Python framework for collecting software metrics from automated builds {#Bitten:_A_Python_framework_for_collecting_software_metrics_from_automated_builds}

Student: Christopher Lenz

Mentors: Greg Wilson, Trent Mick.

Web page: [Bitten](http://bitten.cmlenz.net){.http}
...
Lots of good code, very good time management and prioritization, and he\'s delivered a working system. (Lots still to be done, of course, but it\'s up and running.) See [http://bitten.cmlenz.net](http://bitten.cmlenz.net){.http} for details.

# OpenExVis - A Program Visualization Tool {#OpenExVis_-_A_Program_Visualization_Tool}
...
The project website, where you can also find the original proposal sent to Google, is at [http://openexvis.sourceforge.net/](http://openexvis.sourceforge.net/){.http} and the progress during Summer of Code is tracked in Tero\'s blog at [http://www.teroajk.net/blog/](http://www.teroajk.net/blog/){.http} .
...
Many additions to Wax were released in these two months, including WaxRF (a system to load forms from XML, much like wxPython\'s XRC), a documentation viewing/generating tool, and a number of new controls. Some existing issues were also fixed ([OverlaySizer](./OverlaySizer.html){.nonexistent}, Wizard). More information at [http://zephyrfalcon.org/weblog2/arch_e10_00810.html#e817](http://zephyrfalcon.org/weblog2/arch_e10_00810.html#e817){.http}.

# Data Serving/Collection Framework in Python/WSGI {#Data_Serving.2FCollection_Framework_in_Python.2FWSGI}

Student: Ho Chun Wei

Weblog: [http://cwho.blogspot.com/](http://cwho.blogspot.com/){.http}
...
# Python Bayesian Network Toolbox {#Python_Bayesian_Network_Toolbox}

Student: Elliot Cohen

Mentor: James Tauber

Web page: [PBNT -- Python Bayesian Network Toolbox](http://pbnt.berlios.de/){.http}
...
For (almost) daily updates please see [http://elliotpbnt.blogspot.com](http://elliotpbnt.blogspot.com){.http}.
...
# Efficiently Analysing Data Polymorphism and Deducing Generics in Shedskin {#Efficiently_Analysing_Data_Polymorphism_and_Deducing_Generics_in_Shedskin}
...
# Interactive Python Notebooks {#Interactive_Python_Notebooks}

Students:

- Toni Alatalo, [http://an.org/programming](http://an.org/programming){.http}, Documnet transformation functionality.
...
Original proposal: [http://ipython.scipy.org/google_soc/ipnb_google_soc.pdf](http://ipython.scipy.org/google_soc/ipnb_google_soc.pdf){.http}.

Further details: [http://www.scipy.org/wikis/featurerequests/NoteBook](http://www.scipy.org/wikis/featurerequests/NoteBook){.http}
...
svn co [http://ipython.scipy.org/svn/ipython/nbshell/trunk](http://ipython.scipy.org/svn/ipython/nbshell/trunk){.http} nbshell
...
[http://projects.scipy.org/ipython/ipython](http://projects.scipy.org/ipython/ipython){.http}

# Porting \_sre.c and arraymodule.c to Python {#Porting__sre.c_and_arraymodule.c_to_Python}

Student: Niklaus Haldimann

Weblog: [http://ubique.ch/soc](http://ubique.ch/soc){.http}

Mentors: Armin Rigo, Samuele Pedroni

## Proposal {#Proposal-9}

I would like to create a port of the standard library modules \"\_sre\" and \"array\" to pure Python. This will benefit alternative Python implementations like [PyPy](PyPy), Jython and [IronPython](IronPython). These projects all have to provide their own implementations of standard library modules written in C if they\'re not available in pure Python.

## Outcome {#Outcome-9}

Niklaus Haldimann has done everything planned and more on writing a pure Python implementation of the \_sre (core of regular expressions) and array modules ([http://codespeak.net/svn/user/nik](http://codespeak.net/svn/user/nik){.http}). In addition to the base work, he made regular releases of the \_sre module (three so far). This is a drop-in replacement for the equivalent C modules; for example, \_sre.py could be used in projects that need to run on top of Python 2.3 or earlier, while using regexps that require the recursion-less approach introduced in 2.4 and supported by \_sre.py.

He has also integrated the modules with [PyPy](PyPy) (he was present at two [PyPy](PyPy) sprints); in the case of the \_sre module, he ported it to \"interpreter-level\", which means that in addition to integration, he made sure that the code is static enough to be automatically translatable to good C code, like the rest of [PyPy](PyPy). This should give an excellent write-once, translate-to-anything regular expression engine that might also be used as the basis for a Jython or [IronPython](IronPython) module (or even back to a C extension module for CPython).

# Object-Oriented File System Virtualisation {#Object-Oriented_File_System_Virtualisation}
...
# Wax GUI for Python {#Wax_GUI_for_Python}
...
Web page: [PyTrails](http://seul.org/~jennifer/proj/pytrail/){.http}

## Proposal {#Proposal-12}

I\'m working on an extensible opensource engine for implementing trail-style games such as [Oregon Trail](http://www.gamespot.com/gamespot/features/all/greatestgames/p-34.html){.http} or Amazon Trail. The primary goal is to produce a quality edutainment title that can be used free of cost. The secondary goal is to make it easy for other edutainment trail games to be created. [PyTrails](PyTrails) will be Python based and uses [PyGame](PyGame). The engine will allow following a branching map including making stops to rest, hunt, or trade. Additional choices such as shopping and fording rivers may be available at special points. Each of these activities will be replacable in other trail games as to allow for maximum flexibility.

# mmpy \-- A garbage collection tool kit in Python {#mmpy_--_A_garbage_collection_tool_kit_in_Python}
...
[The student\'s assessment](http://starship.python.net/crew/mwh/blog/nb.cgi/view/nickblog/2005/09/01/0){.http}
...
Library releases available from: [http://developer.berlios.de/project/showfiles.php?group_id=4124](http://developer.berlios.de/project/showfiles.php?group_id=4124){.http}

# Profile Replacement {#Profile_Replacement}

Student: Floris Bruynooghe

Weblog: [http://bruynooghe.blogspot.com/](http://bruynooghe.blogspot.com/){.http}

Mentor: Brett Cannon

## Proposal {#Proposal-16}

\[Original idea from [ProfileReplacementProject](ProfileReplacementProject) page.\]

The current profiler is not free according to the Debian Free Software Guidelines ([http://bugs.debian.org/293932](http://bugs.debian.org/293932){.http}) and has been taken out of the main Debian distribution. This affects many users as the profiler is integrated into other programs such as ipython who lose functionality withouth the profiling available.
...
The project is registered as pyprof on savannah.nongnu.org: [http://savannah.nongnu.org/projects/pyprof](http://savannah.nongnu.org/projects/pyprof){.http}
...
# PythonModulePackaging {#PythonModulePackaging}

Student: Vincenzo Di Massa

URL: [http://udu.wiki.ubuntu.com/PythonModulePackaging](http://udu.wiki.ubuntu.com/PythonModulePackaging){.http}

The above is a dead link. The student refers to [http://easy-deb.sourceforge.net/](http://easy-deb.sourceforge.net/){.http}

Mentor: Matthias Klose

## Proposal {#Proposal-17}
...
# Refactor Pirate (Python on Parrot) {#Refactor_Pirate_.28Python_on_Parrot.29}

Student: Curtis Hall Mentor: Michal Wallace

URL: [http://pirate.tangentcode.com/](http://pirate.tangentcode.com/){.http}

## Proposal {#Proposal-18}

This project didn\'t get mentioned here before. Probably because it was funded through the Perl foundation. ![:)](/wiki/europython/img/smile.png ":)"){height="16" width="16"}
