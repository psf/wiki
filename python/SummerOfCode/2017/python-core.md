...
Python is a popular high-level programming language. It is a general-purpose language used by scientists, developers, and many others who want to get things done quickly and effectively. In this spirit, our primary project this year is about helping make Python faster! Python is a language that can be interpreted and implemented in different ways. The [reference implementation](https://en.wikipedia.org/wiki/Reference_implementation){.https} of Python is [CPython](https://en.wikipedia.org/wiki/CPython){.https}, so named because it\'s written in C. But it\'s not the only option! [PyPy](http://pypy.org/){.http} is an implementation of Python written in Python that prides itself on a great performance. [Jython](http://www.jython.org/){.http} is a Python implementation that compiles Python code to Java bytecode. [IronPython](http://ironpython.net/){.http} is an implementation of Python for the .NET framework. And there are others.
...
Since this year\'s project is focused on performance, you should probably [Take a look at the PyPy benchmarks](http://speed.pypy.org/){.http} and the [Python Performance Benchmark Suite](https://github.com/python/performance){.https} \-- what can you learn about the performance of python? Can you run those benchmarks yourself and see if you get similar results? Try comparing your results to another applicant\'s and see if you both can learn!
...
- [#python-gsoc](http://webchat.freenode.net/?channels=python-gsoc){.http} on irc.freenode.net

Please join us there to chat with your fellow students and mentors! If you\'re new to IRC, you might want to check out [http://www.irchelp.org/](http://www.irchelp.org/){.http}

# Ideas {#Ideas}

## Improve performance benchmarks and performance in Python implementations {#Improve_performance_benchmarks_and_performance_in_Python_implementations}
...
**Skills**: Python experience, as well as experience in other languages used for python implementations (e.g. C for CPython). At least basic understanding of computer performance issues, such as what sort of tasks will be limited by memory, CPU, etc. ([Here\'s a simplistic article about those basics](https://hubpages.com/technology/Improving-the-Performance-of-your-Computer){.https}. If that all sounds familiar to you, you\'re good to go, if it doesn\'t, do some research before applying.)
...
**Related Reading**: The [PyPerformance documentation](http://pyperformance.readthedocs.io/){.http} is a good place to start.

Take a look at [https://speed.python.org/](https://speed.python.org/){.https} and [the PyPy benchmarks](http://speed.pypy.org/){.http}. Try getting copies of existing performance tests and running them on your own machine as a warm-up.

You probably also want to take a look at [the python speed list](https://mail.python.org/mailman/listinfo/speed){.https} ([archives](https://mail.python.org/pipermail/speed/){.https}) to see hear both the latest news and the history of python speed testing.
