...
### Overhaul struct + PEP 3118 {#Overhaul_struct_.2B-_PEP_3118}

There are many open issues for the struct module, and [a suggestion](http://bugs.python.org/msg81911){.http} that many others fixes should happen. The problems include inconsistencies in input handling and backwards-compatibility code that should be gone from 3.x.

As [PEP 3118](http://www.python.org/dev/peps/pep-3118/){.http} also requires [a few changes in struct](http://bugs.python.org/issue3132){.http}, finishing the [memoryview object implementation](http://bugs.python.org/issue2394){.http} and [backporting it to 2.7](http://bugs.python.org/issue2396){.http} could be part of this project.
...
### Clean up and improve the socket module {#Clean_up_and_improve_the_socket_module}
...
### Helper Python core development tools {#Helper_Python_core_development_tools}
...
### Linux package repositories on PyPI {#Linux_package_repositories_on_PyPI}

The project should organize the Linux binary packages on PyPI ([http://pypi.python.org](http://pypi.python.org){.http}) in such a way that they can serve as a Linux package repository, for relevant Linux distributions. It should in particular provide Debian/apt repositories and RPM/yum repositories. Compatibility of packages with certain distributions should be considered.

Contact: Martin v. Löwis \<[martin@v.loewis.de](mailto:martin@v.loewis.de){.mailto}\>

### Roundup / Python Tracker enhancements {#Roundup_.2F_Python_Tracker_enhancements}
...
There are many open developement ideas that could be part of a GSoC proposal. Examples include cleaning up and improving the base infrastructure, performance improvements (DB, web interface), improving end-user UI and features, adding developer-focused features to Roundup and administration enhancements. See [this thread](http://mail.python.org/pipermail/python-dev/2009-March/087560.html){.http} for some examples.
