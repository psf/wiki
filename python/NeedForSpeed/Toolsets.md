...
# Preparation and Tools {#Preparation_and_Tools}

\* Checkout and build CPython:

- svn co [http://svn.python.org/projects/python/trunk](http://svn.python.org/projects/python/trunk){.http} python-svn

\* Checkout and browse pysco:

- svn co [http://codespeak.net/svn/psyco/dist/](http://codespeak.net/svn/psyco/dist/){.http} psyco-dist

\* Sign the [Python contributor agreement](http://www.python.org/psf/contrib-form.html){.http} *Steve and Raymond will have copies at the Sprint*
...
- [FredrikLundh](FredrikLundh): How about IM tools too? Google Talk? (I\'m no IRC user, so maybe modern IRC tools cover point-to-point communication too? Please enlighten me).

  [SeanReifschneider](SeanReifschneider): IRC \*CAN\* do person-to-person talk, as well as small or ad-hoc groups of people, much like IM. You can \"/msg user\" to send them a private message, in xchat that opens a new tab for user-to-user communication. You can \"/join #channel\" to create a new channel named \"channel\" that you can invite others to. I can do IM, please ask me directly for my IM name in AIM or Google Talk, Python folks are welcome to it, but I keep it kind of private.

  [GeorgBrandl](./GeorgBrandl.html){.nonexistent}: Would a kind of collaborative text editor be helpful? For non-Macintosh users, [Gobby](http://gobby.0x539.de){.http} comes to mind.

  [SteveHolden](SteveHolden): I think we can probably do without anything so fancy - pair programming can be the major staple, probably.
