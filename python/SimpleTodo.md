...
## Clean up 64-bit compiler warnings {#Clean_up_64-bit_compiler_warnings}

There are many 64-bit warnings produced by icc when using the -Wp64 flag. icc is freely available for a one month trial license. [NealNorwitz](./NealNorwitz.html){.nonexistent} has a license and can make the warnings available on the web if anyone is interest in getting rid of these warnings. In addition, there is a patch to gcc to produce such warnings.

## Make modules Py_ssize_t clean {#Make_modules_Py_ssize_t_clean}
...
## Check for consistent memory API usage {#Check_for_consistent_memory_API_usage}

[http://mail.python.org/pipermail/python-dev/2006-March/062848.html](http://mail.python.org/pipermail/python-dev/2006-March/062848.html){.http}
...
## Verify all int/long C APIs are correct {#Verify_all_int.2Flong_C_APIs_are_correct}
...
## Add Py_LOCAL, Py_LOCAL_INLINE declarations {#Add_Py_LOCAL.2C_Py_LOCAL_INLINE_declarations}
...
## Create tests for sequences over 2GB {#Create_tests_for_sequences_over_2GB}
...
See [http://python.org/sf/1471578](http://python.org/sf/1471578){.http}

# Test suite improvements {#Test_suite_improvements}
