...
# PEP: Python3 and UnicodeDecodeError {#PEP:_Python3_and_UnicodeDecodeError}
...
# Python3 and UnicodeDecodeError for the command line, environment variables and filenames {#Python3_and_UnicodeDecodeError_for_the_command_line.2C_environment_variables_and_filenames}
...
Python uses \"UTF-8\" as the default Unicode encoding. You can read the default charset using sys.getdefaultencoding(). The \"default encoding\" is used by [PyUnicode](./PyUnicode.html){.nonexistent}\_[FromStringAndSize](./FromStringAndSize.html){.nonexistent}().

A function sys.setdefaultencoding() exists, but it raises a [ValueError](./ValueError.html){.nonexistent} for charset different than UTF-8 since the charset is hardcoded in [PyUnicode](./PyUnicode.html){.nonexistent}\_[FromStringAndSize](./FromStringAndSize.html){.nonexistent}().
...
### Non Windows (POSIX) {#Non_Windows_.28POSIX.29}
...
\"mbcs\" is not a valid charset name, it\'s an internal charset saying that Python will use the function [MultiByteToWideChar](./MultiByteToWideChar.html){.nonexistent}() to decode bytes to unicode. This function uses the current codepage to decode bytes string.
...
[PyUnicode](./PyUnicode.html){.nonexistent}\_DecodeFSDefaultAndSize() uses the default file system encoding, or UTF-8 if it is not set.
...
### Display a filename {#Display_a_filename}
...
### Functions producing filenames {#Functions_producing_filenames}
...
### Functions for filename manipulation {#Functions_for_filename_manipulation}
...
### Functions accessing files {#Functions_accessing_files}
