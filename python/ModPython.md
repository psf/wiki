...
- **URL:** [http://www.modpython.org/](http://www.modpython.org/){.http}
...
See the mod_python [mailing list](http://www.modpython.org/pipermail/mod_python/){.http}.

### ModPython Compatible Web Application Frameworks {#ModPython_Compatible_Web_Application_Frameworks}
...
- [Aquarium](Aquarium) is a MVC web application framework using [Cheetah](http://cheetahtemplate.org){.http} for templating with some resemblance to Struts and a ModPython WSAdaptor.

**Frameworks designed specifically for ModPython:**

- [Pylets](http://pylets.sf.net/){.http} is a new web framework with aspect-oriented [MetaClasses](./MetaClasses.html){.nonexistent}.

- [PyWork](http://pywork.sf.net/){.http} is a Model-View-Controller framework with similarities to Struts.

- [JOTWeb](http://jotweb.tummy.com/){.http} also separates the view and the model in part by using TAL templates.

- [Django](http://www.djangoproject.com/){.http} runs in stand-alone mode for development purpose, but relies on ModPython for production.
...
mod_python provides a php-like built-in templating system called Python Server Pages (PSP), but there are many others that can also be used ([Cheetah](http://www.cheetahtemplate.org/){.http} for example).
...
It is my pleasure to announce that Mod_Python has been donated to the Apache Software Foundation, and is now a subproject of the httpd server project (see [http://httpd.apache.org/](http://httpd.apache.org/){.http}).
...
- [Modpython Tutorial](http://webpython.codepoint.net/mod_python){.http} - An introduction to modpython\'s high level handlers: Publisher and PSP.
:::
