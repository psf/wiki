...
# Concealing (\"Protecting\") Source Code {#Concealing_.28.22Protecting.22.29_Source_Code}
...
(Note that \"freeware\" should **not** be confused with Free Software - see the [Free Software Foundation site](http://www.fsf.org/){.http} for more information on what Free Software is.)
...
## Use Compiled Bytecode {#Use_Compiled_Bytecode}

Python produces `.pyc`{.backtick} files when programs are run for imported modules. This bytecode is not trivially understandable by most developers, and supplying only the bytecode might be sufficient in deterring modification of the code, but there are ways to \"decompile\" the bytecode and recover a human-readable program. The [decompyle](http://sourceforge.net/projects/decompyle/){.http} program is probably the best known tool for this task.

Note that `.pyc`{.backtick} files are not portable between different versions of Python.

## Executable Creators (or Installers) {#Executable_Creators_.28or_Installers.29}
...
## Python Source Code Obfuscators {#Python_Source_Code_Obfuscators}
