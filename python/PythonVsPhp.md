...
# Python vs. PHP {#Python_vs._PHP}

This page was copied mindlessly from [http://wiki.w4py.org/python-vs-php.html](http://wiki.w4py.org/python-vs-php.html){.http} ([Google Cache](http://72.14.203.104/search?q=cache:bo4LWB1ZHswJ:wiki.w4py.org/python-vs-php.html+php+vs+python&hl=en&gl=us&ct=clnk&cd=1){.http})
...
- are [OpenSource](OpenSource) (except where various [Zend products](http://www.zend.com/products){.http}, recommended by some, are employed)
...
## Compared as Languages {#Compared_as_Languages}

### What strengths do PHP have that Python doesn\'t? {#What_strengths_do_PHP_have_that_Python_doesn.27t.3F}
...
- the ternary operator/statement (\... ? \... : \...)
  - Retort: *Python 2.5 has [conditional expressions](http://www.python.org/dev/peps/pep-0308/){.http}*
...
### What weaknesses does PHP have that Python doesn\'t? {#What_weaknesses_does_PHP_have_that_Python_doesn.27t.3F}
...
### What does Python have that PHP doesn\'t? {#What_does_Python_have_that_PHP_doesn.27t.3F}
...
- lots of high-level data types (lists, tuples, dicts, [DateTimes](./DateTimes.html){.nonexistent}, [NumPy](NumPy) arrays, etc.)
...
- differentiation between arrays (lists) and associative arrays (dictionaries).
  - Retort: *PHP array is more flexible. In case developer want a true array, just use [FixedArray](./FixedArray.html){.nonexistent} in PHP SPL*

- support for all major GUI frameworks

- strong internationalization and UNICODE support
  - Retort: *[PHP 6](http://wiki.pooteeweet.org/PhP60){.http} will include Unicode support. This feature is available in PHP5.2 and PHP5.3 via intl extension which can be found at pecl.php.net*
...
## Compared as Web Development Frameworks {#Compared_as_Web_Development_Frameworks}
...
- [http://www.zend.com](http://www.zend.com/){.http}
...
Although both PHP and Python have excellent core documentation, Python\'s is more extensive and generally higher quality. PHP has a large number of translations available. Python doesn\'t. For PHP see [http://www.php.net/docs.php](http://www.php.net/docs.php){.http} and for Python see [http://python.org/doc/](http://python.org/doc/){.http} Python allows documentation on modules, classes, and functions to be included in the program code. The documentation becomes an attribute of the module/class/function, accessible from inside of the language itself. Python manual is really awfully structurized and presented compared to PHP manual, which uses cross-links, a lot of colorized examples and invaluable user comments to make it easier to comprehend the magic. PHP manual merges different versions of the language together making it a little bit bloated.
...
## Constructive Criticism / Observations {#Constructive_Criticism_.2F_Observations}
...
### Editorial Response {#Editorial_Response}
...
## Using Python and PHP together {#Using_Python_and_PHP_together}

The [Embedded Python](http://pecl.php.net/package/python){.http} [PECL](http://pecl.php.net/){.http} extension \"allows the Python interpreter to be embedded inside of PHP, allowing for the instantiate and manipulation of Python objects from within PHP\". The PyPI [phpserialize](http://pypi.python.org/pypi/phpserialize/){.http} package is \"a port of the serialize and unserialize functions of php to python\". [phppython](http://code.google.com/p/phppython/){.http} \"is a php to python code converter that currently can: convert small php code snippets, functions to python code\". [WPHP](http://pythonpaste.org/wphp/){.http} is a WSGI-\>PHP gateway that \"allows you to run PHP processes inside of Python, using a WSGI gateway\". Alternatively take a look at phpython [https://sourceforge.net/projects/phpython/](https://sourceforge.net/projects/phpython/){.https}, a Python interpreter written entirely in PHP, to be used without the aid of WSGI, CGI or FastCGI
:::
