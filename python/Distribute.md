...
# Distribute is a project that tries to think about how distutils can evolve {#Distribute_is_a_project_that_tries_to_think_about_how_distutils_can_evolve}

Interesting discussion are happening in distutils-SIG. You can read the threads

- [Initial thread](http://mail.python.org/pipermail/distutils-sig/2008-September/010031.html){.http}

- [Guido answer, followed by other answers](http://mail.python.org/pipermail/distutils-sig/2008-September/010080.html){.http}
...
Another great ressource to get the whole picture is [Kevin Teague\'s post](http://groups.google.com/group/django-developers/msg/5407cdb400157259){.http} in Django Mailing list.

Chris Withers made a presentation called [Python Package Management Sucks](http://www.simplistix.co.uk/presentations/python_package_management_08/python_package_management_08.pdf){.http} at [PyCon](PyCon) Uk in september 2008.

# Related Docs {#Related_Docs}

- [Functionality](http://wiki.python.org/moin/Distribute/Functionality){.http}

- [DistributeSprint #1](./DistributeSprint(20).html#A1){.nonexistent} : Starting a PEP series

- [Packaging Survey](./Packaging(20)Survey.html) : Survey for the Python Language Summit 2009

- [distutils-autoconf](./distutils(2d)autoconf.html){.nonexistent} : Proposal by David

- [Defend Against Fruit Docs](https://github.com/teamfruit/defend_against_fruit/wiki){.https}: Continuous Deployment considerations and Python package management

# Related PEPs {#Related_PEPs}

- a new package dedicated to PyPI handling [PEP 373](http://wiki.python.org/moin/A_new_pypi_module){.http}

- mirroring protocol for PyPI : [PEP 374](./PEP(20)374.html){.nonexistent}

# What do we have today ? {#What_do_we_have_today_.3F}
...
Right now in Python, Distutils provides a set of commands to create a distribution of your package. It uses a set of metadata that describes the package and build an archive with the source code. These metadata are described in [PEP 345](http://www.python.org/dev/peps/pep-0345/){.http}.
...
Setuptools can be seen as an enhancement of Distutils. It does a lot of things, and this section will not present everything. You might want to read [Phillip\'s page on the project](http://peak.telecommunity.com/DevCenter/setuptools){.http}.
...
### real-world example {#real-world_example}
...
And if you try to install it, Setuptools will check if it is installed. If not it will try to get it at [PyPI](http://pypi.python.org){.http} and instal it.
...
[http://www.blueskyonmars.com/projects/paver](http://www.blueskyonmars.com/projects/paver){.http}
...
# What is good ? {#What_is_good_.3F}

## Distutils {#Distutils-1}
...
## Other tools {#Other_tools-1}

xxx

# What is wrong ? {#What_is_wrong_.3F}
...
## Other tools {#Other_tools-2}

xxx

# What can we do today ? {#What_can_we_do_today_.3F}
...
## Other tools {#Other_tools-3}

xxx

# What\'s next ? {#What.27s_next_.3F}
...
- [OsVendorsRequirements](./OsVendorsRequirements.html){.nonexistent}
:::
