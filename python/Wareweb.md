...
:   [http://pythonpaste.org](http://pythonpaste.org){.http}
...
A WSGI-based framework, intended to be used under [Paste](http://pythonpaste.org/){.http}.
...
Includes its own minimal event framework, and metaclass protocol, to make plugins/components fairly simple and general. In this case, a plugin can be written to handle templating. Only one such templating language is supported currently, in [ZPTKit](http://imagescape.com/software/ZPTKit/){.http}.

### InTheirOwnWords {#InTheirOwnWords}

Wareweb is a refactoring of the programming model of [Webware](./Webware.html). It\'s intended to keep the same conceptual model (servlets), while natively including extensions (like [Component](http://wiki.w4py.org/component.html){.http}, and removing some aspects of Webware that were never flushed out.
