...
The [Wikipedia Weekly](http://wikipediaweekly.org/){.http} podcast is a good model for us. Try listening to episode 80 or 81, which are daily wrap-ups from the Wikimania 2009 conference, or episode 78, which is a Skype-recorded episode.
...
## How to record Skype audio {#How_to_record_Skype_audio}

[Skype for Interviews](http://www.blogarithms.com/index.php/archives/2007/12/23/skype-for-interviews/){.http} \-- how-to video

[15 Apps for Recording Skype Conversations](http://www.voip-sol.com/15-apps-for-recording-skype-conversations/){.http} \-- Windows/MacOS

Linux tool: [Skype Call Recorder](http://atdot.ch/scr/){.http}

[Record Skype Calls on Ubuntu](http://www.detector-pro.com/2008/08/how-to-record-skype-calls-on-ubuntu.html){.http} (from 2008)

[Recording Skype on Linux](http://www.andrlik.org/blog/2006/feb/17/howto-record-skype-conversations-in-linux/){.http} (from 2006)

We should write a detailed set of instructions here for each platform.

## MacOS {#MacOS}

[Call Recorder](http://www.ecamm.com/mac/callrecorder/){.http} seems to work smoothly.

To use only free software, follow the instructions from [http://zeljkofilipin.com/2009/03/17/recording-a-podcast-with-skype-on-mac-using-only-free-software/](http://zeljkofilipin.com/2009/03/17/recording-a-podcast-with-skype-on-mac-using-only-free-software/){.http} . (These instructions work to make a recording but are not usable, since when you\'re talking, you hear your own voice in your headphones, but with a large enough delay that it\'s really hard to concentrate. \--amk)

Install Audacity, [LineIn](./LineIn.html){.nonexistent}, and Soundflower (all are free software).
...
### LineIn Application {#LineIn_Application}

Input from: Built-in Audio: Line In (ie the Mic) Output to [SoundFlower](./SoundFlower.html){.nonexistent} (2ch)

And then click the 'Pass Thru' Button in [LineIn](./LineIn.html){.nonexistent} to actually begin transferring audio between the input and output.
