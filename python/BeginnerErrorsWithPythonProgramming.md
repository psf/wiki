...
## Common Problems Beginners Have {#Common_Problems_Beginners_Have}

- The [Alice](http://www.alice.org/){.http} 3D project developed a 3D programming environment for beginners. Originally, they used [Python](Python) but later switched to another language. Two Python-specific issues were found to be problematic for beginners: (1) the case-sensitivity of the language (\"variable1\" is not the same as \"Variable1\"), and (2) integer division (a carryover from the programming language C, 3 / 4 = 0, not 0.75, because the division of integers returns integers). Python 3 changed the latter by introducing an explicit integer division operator, so now 3 / 4 is 0.75 and 3 // 4 = 0. The former is just part of the language. See [interview](http://www.linuxjournal.com/article.php?sid=5028){.http} in Linux Journal and this [dissertation](http://www.alice.org/wp-content/uploads/2017/04/ConwayDissertation.pdf){.http} for more details.
...
- Widely known as the \"No Module named\...\" error, this is an issue all programmers face at some point (sometimes multiple times). This article([The \"No Module Named\" Error](https://coderslegacy.com/python-no-module-named-import-error/){.https}) addresses this issue, going through every possible scenario in which this error occurs and how to avoid running into it again.
...
## Comments, Potential Solutions {#Comments.2C_Potential_Solutions}

- The [PyChecker](http://pychecker.sourceforge.net/){.http} module can alert users to many common errors. There also is a tool called [PyLint](https://www.pylint.org/){.https}.

- To see languages that have been used successfully with beginners and children, see Logo, [HyperTalk](./HyperTalk.html){.nonexistent} (used in [HyperCard](./HyperCard.html){.nonexistent} and [SuperCard](./SuperCard.html){.nonexistent}), HTML, [JavaScript](./JavaScript.html){.nonexistent}, variants of Basic, etc.
