...
# Python 2.0 FAQ {#Python_2.0_FAQ}

::: table-of-contents
Contents

1.  [Python 2.0 FAQ](#Python_2.0_FAQ)
    1.  [Win 95 Installation](#Win_95_Installation)
    2.  [C to Python API](#C_to_Python_API)
    3.  [Floating Point](#Floating_Point)
    4.  [LONG_BIT Error](#LONG_BIT_Error)
    5.  [pyexpat Module](#pyexpat_Module)
    6.  [Where\'s IDLE?](#Where.27s_IDLE.3F)
    7.  [\"make test\" reports failures](#A.22make_test.22_reports_failures)
    8.  [Documentation in Windows Helpfile Format](#Documentation_in_Windows_Helpfile_Format)
    9.  [Where\'s the IDE for Python?](#Where.27s_the_IDE_for_Python.3F)
    10. [libreadline.so.3](#libreadline.so.3)
    11. [Windows 2K](#Windows_2K)
    12. [RedHat 7.0](#RedHat_7.0)
    13. [Extensions using C++ compiled on Solaris with G++](#Extensions_using_C.2B-.2B-_compiled_on_Solaris_with_G.2B-.2B-)
:::

Add your questions here and I\'ll do my best to answer them.

See also [CriticalPatches](./CriticalPatches.html){.nonexistent} and [MiscPatches](./MiscPatches.html){.nonexistent}.
...
## C to Python API {#C_to_Python_API}
...
**A.** You must be using Red Hat 7.0. It comes with a broken compiler. GCC 2.96 was never intended for distribution. You will need to update your glibc to version 2.2 (see [http://www.redhat.com/support/errata/RHBA-2000-079.html](http://www.redhat.com/support/errata/RHBA-2000-079.html){.http} for more information).
...
## Where\'s IDLE? {#Where.27s_IDLE.3F}

**Q.** I just downloaded version 2.0 (BeOpen-Python-2.0-1.i386.rpm) and there is no trace of IDLE in there; an \"`rpm -qpl BeOpen-Python-2.0-1.i386.rpm | grep -i idle`\" comes up empty. [http://www.python.org/idle/](http://www.python.org/idle/){.http} claims that IDLE 0.6 is distributed as part of Python 2.0. Checking further, I see that IDLE is included in [ftp://ftp.python.org/pub/python/src/python-2.0c1.tar.gz](ftp://ftp.python.org/pub/python/src/python-2.0c1.tar.gz){.ftp} in the tools directory. Is this an oversight? Is there a way to install IDLE without unpacking the source tarball? ([bob@drzyzgula.org](mailto:bob@drzyzgula.org){.mailto})

**A.** This is an oversight. None of the code in Tools is installed. I will fix this as soon as possible. ([jeremy@alum.mit.edu](mailto:jeremy@alum.mit.edu){.mailto})

(Postscript: I don\'t think this was ever fixed.)

------------------------------------------------------------------------

## \"make test\" reports failures {#A.22make_test.22_reports_failures}
...
The question is, what do I do to resolve these failures or are they insignificant? ([gdinwiddie@min.net](mailto:gdinwiddie@min.net){.mailto}, Python newbie)
...
Note that if you suspect a\'bug\' in Python (rather than a bug in your platform), the proper place to go to is the bug manager: [http://bugs.python.org/](http://bugs.python.org/){.http} . ([guido@python.org](mailto:guido@python.org){.mailto})
...
I hope this is of interest. ([gdinwiddie@min.net](mailto:gdinwiddie@min.net){.mailto})
...
**A.** Robin Dunn [announced](http://www.deja.com/=dnc/getdoc.xp?AN=683846117.1 "DejaNews"){.interwiki} that he created a ZIP file with the BeOpen Python 2.0 docs, which you can download at \"[http://alldunn.com/python/py2docs.zip](http://alldunn.com/python/py2docs.zip){.http}\". This also includes the tool (written in Python, of course) to generate the project files for the Microsoft Help Workshop.

**A.** See also \"[http://www.orgmf.com.ar/condor/pyshelf21.zip](http://www.orgmf.com.ar/condor/pyshelf21.zip){.http}\" !

------------------------------------------------------------------------

## Where\'s the IDE for Python? {#Where.27s_the_IDE_for_Python.3F}
...
**A.** Commercial offerings: For Windows and Linux, there are several: PythonWorks ( [http://www.pythonworks.com](http://www.pythonworks.com){.http} ); Wing IDE ( [http://www.wingide.com](http://www.wingide.com){.http} ); BlackAdder ( [http://www.thekompany.com/products/blackadder/](http://www.thekompany.com/products/blackadder/){.http} ).

In the free (what you get is not necessarily what you pay for) department: For Windows, there\'s also Pythonwin, which comes as part of ActivePython ([http://www.activestate.com](http://www.activestate.com){.http}). For Mac OS (Classic and X), the standard [MacPython](MacPython) installation includes an IDE (\'Python IDE\'). For any platform with Tcl/Tk, there\'s also IDLE, which comes standard with Python ([http://www.python.org/idle/](http://www.python.org/idle/){.http}).

In the future, expect more offerings, e.g. VisualPython (a Visual Studio plugin from ActiveState), Komodo (a Mozilla-based cross-platform offering from ActiveState), and Boa Constructor ([http://boa-constructor.sourceforge.net](http://boa-constructor.sourceforge.net){.http}).

See also the FAQTS entry about this topic: [http://www.faqts.com/knowledge-base/view.phtml/aid/6433/fid/199/lang/en](http://www.faqts.com/knowledge-base/view.phtml/aid/6433/fid/199/lang/en){.http}

Update: see [http://www-106.ibm.com/developerworks/linux/library/l-pide/](http://www-106.ibm.com/developerworks/linux/library/l-pide/){.http} for a review of several IDEs mentioned above.
...
## Extensions using C++ compiled on Solaris with G++ {#Extensions_using_C.2B-.2B-_compiled_on_Solaris_with_G.2B-.2B-}

**Q.** Linker errors when building extensions that use C++ on Solaris.

**A.** Martin von Loewis gave an explanation in python-dev: [http://mail.python.org/pipermail/python-dev/2001-March/013510.html](http://mail.python.org/pipermail/python-dev/2001-March/013510.html){.http}

and more: [http://groups.google.com/groups?hl=en&lr=&ie=UTF-8&selm=avsmd9%24mcc%2401%241%40news.t-online.com](http://groups.google.com/groups?hl=en&lr=&ie=UTF-8&selm=avsmd9%24mcc%2401%241%40news.t-online.com){.http}
::::
