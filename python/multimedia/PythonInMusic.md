...
- **[Soundgrain](http://code.google.com/p/soundgrain/)** - Soundgrain is a graphical interface where users can draw and edit trajectories to control granular sound synthesis modules. Soundgrain is written with Python and [WxPython](WxPython) and uses pyo as its audio engine.
...
- **[PyPortMidi](http://web.media.mit.edu/~harrison/pyportmidi.html)** - [PyPortMidi](PyPortMidi) is a Python wrapper for [PortMidi](./PortMidi.html). [PortMidi](./PortMidi.html) is a cross-platform C library for realtime MIDI control. Using [PyPortMidi](PyPortMidi), you can send and receive MIDI data in realtime from Python. Besides using [PyPortMidi](PyPortMidi) to communicate to synthesizers and the like, it is possible to use [PyPortMidi](PyPortMidi) as a way to send MIDI messages between software packages on the same computer. [PyPortMidi](PyPortMidi) is now maintained at [http://bitbucket.org/aalex/pyportmidi/](http://bitbucket.org/aalex/pyportmidi/)
