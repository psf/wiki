...
**encoding**
:   In Python documentation, this is short for [character encoding](http://en.wikipedia.org/wiki/Character_encoding){.http .reference .external}. In the email RFCs, encoding is short for \"transfer encoding\", and refers to the way in which arbitrary bytes are encoded into US-ASCII so as to produce an RFC conformant byte stream. The RFC defined encodings are \"quoted printable\" and \"base64\".
...
:   Data in the form it enters the email module parser, or exits the email module generator (primarily the former). [\[\*\]](#id2){#id1 .footnote-reference} A related term is \'wire-format\', the difference being that wire-format data is understood to be RFC conformant. Raw data may or may not be RFC conformant, and may or may not be bytes (if, for example, it comes from a doctest or other text input source).

      ----------------------------- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      [\[\*\]](#id1){.fn-backref}   Note that in the past this term has been used ambiguously to also refer to the original source data that was transfer-encoded into the form that is the actual raw data that the email module deals with.
      ----------------------------- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
