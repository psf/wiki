...
:   [http://www.microsoft.com/sql/default.mspx](http://www.microsoft.com/sql/default.mspx){.http}
...
:   [http://adodbapi.sourceforge.net/](http://adodbapi.sourceforge.net/){.http}

SourceForge

:   [http://sourceforge.net/projects/adodbapi](http://sourceforge.net/projects/adodbapi){.http}
...
:   [http://pymssql.org](http://pymssql.org){.http}
...
:   [http://www.object-craft.com.au/projects/mssql/](http://www.object-craft.com.au/projects/mssql/){.http}
...
:   [http://www.egenix.com/](http://www.egenix.com/){.http}
...
mxODBC requires an ODBC driver to talk to SQL Server. On Windows, you can use the [MS SQL Server Native Client ODBC driver for Windows](http://msdn.microsoft.com/en-us/library/jj991993.aspx){.http}, on the other platforms, there are several commercial ODBC high quality drivers available, an open-source [http://www.freetds.org/ FreeTDS ODBC driver](http://www.freetds.org/%20FreeTDS%20ODBC%20driver){.http} for Unix platforms and the free [MS SQL Server Native Client ODBC driver for Linux x64](http://msdn.microsoft.com/en-us/library/hh568451%28v=sql.110%29.aspx){.http}.
...
Supports ANSI and Unicode data and SQL statements and includes an extensive set of unit tests for SQL Server. pyODBC require ODBC driver to work correctly with SQL Server. You may [download latest SQL Server ODBC driver](https://www.devart.com/odbc/sqlserver/download.html){.https} and use it freely. Or you may choose Microsoft ODBC driver for that needs which is posted above in mxODBC driver description.

### pypyodbc （Pure Python) {#pypyodbc_.2B.2Fwg-Pure_Python.29}
...
[Connect SQL Server in 3 steps with pypyodbc on Linux](http://code.google.com/p/pypyodbc/wiki/Linux_ODBC_in_3_steps){.http}

[Run SQLAlchemy on PyPy with pypyodbc driver](https://code.google.com/p/pypyodbc/wiki/Enable_SQLAlchemy_on_PyPy){.https}
...
*Comment:* This is actually not true at all: ODBC is the native API used for SQL Server and does support Unicode all the way. In fact, ODBC is the preferred way of accessing SQL Server if you care for performance. Microsoft has just released the [SQL Server Native Client](http://msdn2.microsoft.com/en-us/data/aa937733.aspx){.http} which is an extended ODBC driver for SQL Server. ADO is just a layer on top of the ODBC interface and a lot slower as a result. See e.g. [MS TechNet](http://www.microsoft.com/technet/prodtechnol/windows2000serv/technologies/iis/reskit/iischp7.mspx){.http} for a comparison of ODBC, OLE DB and ADO, or [this cookbook entry](http://aspn.activestate.com/ASPN/Cookbook/Python/Recipe/389535){.http}.
