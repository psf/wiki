...
We\'re moving this page to the official [JythonSprint](http://wiki.python.org/jython/JythonSprint){.http} page on the Jython wiki.
...
# Some other targets {#Some_other_targets}

[http://wiki.python.org/jython/JythonDeveloperGuide/PleaseAdoptMe](http://wiki.python.org/jython/JythonDeveloperGuide/PleaseAdoptMe){.http}

# Design Ideas {#Design_Ideas}
...
- Reuse. What can Jython use from the Java ecosystem, especially other dynamic languages. As we will see, this means mostly frameworks for bytecode engineering, specifically [ASM](http://asm.objectweb.org/){.http}.
...
## Keep JavaCC {#Keep_JavaCC}

In the spirit of radical revolution, we looked at the feasibility of using CPython's new AST approach. (AST = abstract syntax tree, think of a specific snippet of Python code as a Lisp expression.) This is documented in [PEP 339](http://www.python.org/dev/peps/pep-0339/){.http}. This was motivated by the idea, why not just lift the grammar -- Python.asdl - from CPython 2.5 exactly. This is elegantly written and would allow for exact compliance. Seems like a nice idea!

Notice that some of Jython was generated using an older version of this new approach see: [http://svn.python.org/view/sandbox/trunk/ast/asdl_java.py?rev=40754&view=log](http://svn.python.org/view/sandbox/trunk/ast/asdl_java.py?rev=40754&view=log){.http}

First, let's look at what's currently happening in Jython. Jython uses [JavaCC](https://javacc.dev.java.net/){.https} and the related project [JJTree](https://javacc.dev.java.net/doc/JJTree.html){.https}. JJTree in particular is used to preprocess the JavaCC grammar (python.jjt), which is then compiled to python.jj, then to [PythonGrammar](./PythonGrammar.html){.nonexistent}.java (a standard YACCish state machine), and all of the AST nodes in ast/\*.java.

CPython 2.5 is using Zephyr's ASDL (Abstract Syntax Definition Language), which is a language for writing tree grammars. At first, we thought that it meant it was using Zephyr code generation -- and this is convenient, asdlGen supports both Java and C generation. But this is not in fact the case. The reason is very sound: to enable introspection of any ASTs, the module asdl_c.py generates nodes for the AST that are made out of standard Python internal data structures like [PyObject](./PyObject.html){.nonexistent}. But of course we are not interested in C data structures here. We also should mention that [Zephyr](http://sourceforge.net/projects/asdl){.http} itself has not been updated in nearly 5 years on [SourceForge](SourceForge).
...
## Move to ASM for Bytecode Generation {#Move_to_ASM_for_Bytecode_Generation}
...
### Removing Reflection {#Removing_Reflection}

ASM also readily enables what Dennis Sosnoski described as [classworking](http://www-128.ibm.com/developerworks/java/library/j-dyn0610/){.http}, a method of replacing reflection with bytecode generation, since this is what ASM directly supports. Comments in the current code, [PyJavaClass](./PyJavaClass.html){.nonexistent}.java, to support reflection suggest there's an [ancient (1997) bug](http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4071957){.http} lurking here. Is it possible to remove reflection then completely in favor of just a bit more bytecode generation? It would seem so.

### Comparative Generation {#Comparative_Generation}

Here's a list of modules that perform bytecode generation in the respective languages:

- Jython. [CodeCompiler](./CodeCompiler.html){.nonexistent}.java

- Groovy. [AsmClassGenerator](./AsmClassGenerator.html){.nonexistent}.java

- CPython. compile.c

Some relevant resources:

- [Java VM specification](http://java.sun.com/docs/books/vmspec/2nd-edition/html/VMSpecTOC.doc.html){.http}

## Fix a Compliance Target {#Fix_a_Compliance_Target}
...
This follows what Charles Nutter observed about JRuby and Rails: [Setting a compelling goal](http://sourceforge.net/mailarchive/forum.php?thread_id=30988621&forum_id=5587){.http}

## Replacing JythonC {#Replacing_JythonC}
...
- [Jar API](http://java.sun.com/j2se/1.4.2/docs/api/java/util/jar/package-summary.html){.http}

- [Jar Jar Links](http://tonicsystems.com/products/jarjar/){.http}

## Supporting select {#Supporting_select}

Various networking modules in the standard library do not work because of a lack of Posix style select. Java NIO has what looks like a close match in [java.nio.channels.Selector](http://java.sun.com/j2se/1.4.2/docs/api/java/nio/channels/Selector.html){.http}.

# Sprint Participation {#Sprint_Participation}

## Engineering {#Engineering}

Charlie Groves (jython committer) suggests:

I\'ve been starting people out with porting a Python module written in C to Jython as described in [http://wiki.python.org/jython/JythonDeveloperGuide/PortingPythonModulesToJython](http://wiki.python.org/jython/JythonDeveloperGuide/PortingPythonModulesToJython){.http}. Paul has been making a nice writeup of porting the csv module in more detail in his blog at [http://gushieblog.blogspot.com/](http://gushieblog.blogspot.com/){.http}
...
# Technical Wish List {#Technical_Wish_List}
...
## Classpath / invocation environment issues {#Classpath_.2F_invocation_environment_issues}
...
## Improving coverage and awareness of Jython environments {#Improving_coverage_and_awareness_of_Jython_environments}
...
## Jython installation package {#Jython_installation_package}
...
- [Developers\' FAQ](http://www.jython.org/Project/devfaq.html){.http}

- [Developers\' Guide](http://www.jython.org/Project/devguide.html){.http}

# Supporting Comments {#Supporting_Comments}
