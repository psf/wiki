...
# PSF Grants Workgroup Charter {#PSF_Grants_Workgroup_Charter}

::: table-of-contents
Contents

1.  [PSF Grants Workgroup Charter](#PSF_Grants_Workgroup_Charter)
    1.  [Purpose & Common Goals](#Purpose_.26_Common_Goals)
    2.  [Guiding Principles](#Guiding_Principles)
        1.  [Program Guiding Principles](#Program_Guiding_Principles)
        2.  [Funding Guiding Principles](#Funding_Guiding_Principles)
    3.  [Active Time](#Active_Time)
    4.  [Core Values & Internal Governance](#Core_Values_.26_Internal_Governance)
        1.  [Membership](#Membership)
    5.  [Grant Application Guidelines](#Grant_Application_Guidelines)
        1.  [Grant Caps by Type](#Grant_Caps_by_Type)
        2.  [Application Criteria](#Application_Criteria)
        3.  [Fast Track Grants](#Fast_Track_Grants)
    6.  [Grant Application Workflow](#Grant_Application_Workflow)
    7.  [Voting Rules](#Voting_Rules)
    8.  [Community Partner Program](#Community_Partner_Program)
        1.  [Promotional Support](#Promotional_Support)
    9.  [Communication Plan](#Communication_Plan)
    10. [Budget](#Budget)
    11. [Contact Information](#Contact_Information)
    12. [List of Participants / Who we are](#List_of_Participants_.2F_Who_we_are)
:::
...
### Program Guiding Principles {#Program_Guiding_Principles}
...
### Funding Guiding Principles {#Funding_Guiding_Principles}
...
The Grants Work Group is governed by the [PSF Code of Conduct](https://www.python.org/psf/codeofconduct/){.https}. Any actions made against the principles in the Code of Conduct will result in a report submitted to the PSF Code of Conduct Work Group. The Chair or Vice Chairs reserve the right to moderate Work Group members on the psf-grants mailing list.
...
## Grant Application Guidelines {#Grant_Application_Guidelines}

### Grant Caps by Type {#Grant_Caps_by_Type}
...
- [PyLadies](./PyLadies.html){.nonexistent} Workshop: \$1,500 USD

- [DjangoGirls](./DjangoGirls.html){.nonexistent} Workshop: \$1,500 USD

- Consolidated grant requests will be capped according to the figures above.
  - Example: Python Neptune is organizing one [PyCon](./PyCon.html){.nonexistent} Conference and 3 workshops in 2025. They would be eligible to receive \$12,500.
...
### Application Criteria {#Application_Criteria}
...
- For any project, event, initiative, or grant applicant that receives funding, the PSF requests an accountability report. For any subsequent grant applications (whether by project/event or by individual), a report for past awards within 3 years is required to consider further requests.
  - Example: Jules, the new lead organizer for [PyCon](./PyCon.html){.nonexistent} Jupiter 2050 and first-time applicant, is requesting a grant for the event. Jules needs to ensure a report has been submitted for [PyCon](./PyCon.html){.nonexistent} Jupiter 2049 for their 2050 application to be considered. The report can be submitted by the previous grant recipient, anyone on the organizing team, or Jules, if they were involved in organizing the 2049 event.

  - Example: Cameron, grant recipient for [PyCon](./PyCon.html){.nonexistent} Mars 2050, applies for [PyLadies](./PyLadies.html){.nonexistent} Neptune 2051. Cameron must submit a report for [PyCon](./PyCon.html){.nonexistent} Mars 2050 for their current [PyLadies](./PyLadies.html){.nonexistent} application to be considered.
...
### Fast Track Grants {#Fast_Track_Grants}
...
- The total requested grant amount is \$2,500 USD or less, and only 10% above the previous amount granted.
  - Example: [PyCon](./PyCon.html){.nonexistent} Mars is approved for a grant of \$2,000 USD in 2022. [PyCon](./PyCon.html){.nonexistent} Mars applies for a \$2,150 USD grant in 2024. Assuming the grant meets all other criteria, the grant application would be fast-tracked.
...
## Grant Application Workflow {#Grant_Application_Workflow}

- Grant applicants submit grant requests through the [PSF Grant Proposal Form](https://psfmember.org/grant-proposals/){.https}.
...
## Community Partner Program {#Community_Partner_Program}
...
- Applicants submit an application for their event or initiative through a Google form.
- The GWG Chair performs a pre-review of the application.
  - Questions will be sent to the applicant as needed.
- The GWG Chair forwards the application with any notes to the GWG for review.
- The GWG reviews the applications and asks questions or provides commentary.
  - Questions will be sent to the applicant as needed.
- The GWG votes via a lazy consensus voting model (3 +1's, and no -1's in the span of 10 days).
- If there are -1's, the GWG Chair facilitates a discussion, and a full consensus must be reached.
...
- Our travel grant/scholarship program is open
...
- Fundraisers or requests for donations
- Registration-related posts
- Anything related to purchases or revenue
- Anything the PSF deems would threaten its non-profit status in the US
...
The GWG Chair maintains the GWG's emails in a PSF-managed mailing list: [psf-grants@python.org](mailto:psf-grants@python.org){.mailto}.
...
Mailing list: [psf-grants@python.org](mailto:psf-grants@python.org){.mailto}

## List of Participants / Who we are {#List_of_Participants_.2F_Who_we_are}
