...
# pip Team Meeting {#pip_Team_Meeting}
...
1.  [pip Team Meeting](#pip_Team_Meeting)
    1.  [Test case formats](#Test_case_formats)
    2.  [Timeline of the resolver\'s rollout](#Timeline_of_the_resolver.27s_rollout)
    3.  [Dealing with metadata](#Dealing_with_metadata)
    4.  [Refactoring in pip](#Refactoring_in_pip)
:::

General TODO: Sumana: consolidate and post past notes (will do this on Friday)

## Test case formats {#Test_case_formats}
...
- Recent pip tests: [https://github.com/pradyunsg/pip/tree/resolver/make-it-inside/tests/yaml/](https://github.com/pradyunsg/pip/tree/resolver/make-it-inside/tests/yaml/){.https}

- Recent resolvelib tests: [https://github.com/sarugaku/resolvelib/tree/master/tests](https://github.com/sarugaku/resolvelib/tree/master/tests){.https}
...
## Timeline of the resolver\'s rollout {#Timeline_of_the_resolver.27s_rollout}

- revising Paul\'s draft announcement at [https://gist.github.com/pfmoore/4034f6077e5f97fbe1a056078ca8f353](https://gist.github.com/pfmoore/4034f6077e5f97fbe1a056078ca8f353){.https}
...
## Dealing with metadata {#Dealing_with_metadata}

- Pradyun: Ilan, can you give advice on how to deal with bad metadata - to give users some mechanisms how to deal with them.

- Ilan: Metadata comes from [PyPi](./PyPi.html){.nonexistent}, sourced from devs who upload packages. conda approach is to have 1 file for metadata. There isn\'t an equivalent in pip.
...
- Paul: The issue with that is that we still have to support [DevPi](./DevPi.html){.nonexistent}, etc \-- which doesn\'t have this richer information. We might still hit issues with this, becuase some of the metadata is dynamic. We struggle to put too much dependence on the static-metadata plans. For this piece of work, we\'ll have to treat that getting-dependency-metadata will be expensive. We\'re going to live with that constraint.
...
## Refactoring in pip {#Refactoring_in_pip}
...
BTW, eww, this colour that got assigned to me is really ugly ![:-)](/wiki/europython/img/smile.png ":-)"){height="16" width="16"}
