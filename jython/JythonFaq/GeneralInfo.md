...
1.  [General Information](#General_Information)
    1.  [What is Jython?](#What_is_Jython.3F)
    2.  [Is Jython the same language as Python?](#Is_Jython_the_same_language_as_Python.3F)
    3.  [What is the current status of Jython?](#What_is_the_current_status_of_Jython.3F)
    4.  [How fast is Jython?](#How_fast_is_Jython.3F)
    5.  [How do I learn more about Jython?](#How_do_I_learn_more_about_Jython.3F)
    6.  [Can I use Jython to make apps for mobile phones?](#Can_I_use_Jython_to_make_apps_for_mobile_phones.3F)
:::

------------------------------------------------------------------------

## What is Jython? {#What_is_Jython.3F}
...
## Is Jython the same language as Python? {#Is_Jython_the_same_language_as_Python.3F}
...
## What is the current status of Jython? {#What_is_the_current_status_of_Jython.3F}
...
## How fast is Jython? {#How_fast_is_Jython.3F}
...
Areas that are known to be slower include the expat emulation (which is used by [ElementTree](./ElementTree.html){.nonexistent}), bisect, datetime, decimal, heapq, and unicodedata, since these are all currently written in Python instead of Java; in CPython, these are all written in C. Future releases of Jytho can address this situation.

If you find a specific area where Jython performance is worse than CPython, and especially if you have a patch, please file a bug at [http://bugs.jython.org](http://bugs.jython.org){.http}.

------------------------------------------------------------------------

## How do I learn more about Jython? {#How_do_I_learn_more_about_Jython.3F}

You might want to start with the the Apress book documenting Jython 2.5, [The Definitive Guide to Jython](http://apress.com/book/view/9781430225270){.http}, which is also available online as the [Jython Book](http://jythonbook.com){.http}, an open source version.
...
- The [official Python documentation](http://python.org/doc/){.http} - (note this covers the 2.6 version of the language and libraries). This is currently being patched against the small changes seen in Jython. Note that class decorators, the ast module, and the namedtuple factory function from 2.6 were implemented in Jython 2.5.0.

- [Dive into Python](http://diveintopython.net){.http} is a great learning resource

for Jython specific starting points try these:

- [The Jython Users Guide](UserGuide)

- An excellent book, [\"Jython Essentials\"](http://www.oreilly.com/catalog/jythoness/){.http}, by Samuele Pedroni & Noel Rappin

- as the song goes, [\"\...getting to know you..\"](http://www.ibm.com/developerworks/java/library/j-alj07064/){.http}

- An excellent [online course](http://www.rexx.com/~dkuhlman/jython_course_03.html){.http} by Dave Kuhlman
...
## Can I use Jython to make apps for mobile phones? {#Can_I_use_Jython_to_make_apps_for_mobile_phones.3F}

Unfortunately not. The Android operating system does use Java as its primary programming language, but there is no known port of Jython to the platform. If you want to make apps for Android smartphones, see the Python Wiki\'s [Android page](https://wiki.python.org/moin/Android){.https} for alternative approaches.

Some older mobile phones have [Java ME (Micro Edition)](http://en.wikipedia.org/wiki/Java_Platform,_Micro_Edition){.http}, but Jython requires [Java SE (Standard Edition)](http://en.wikipedia.org/wiki/Java_Platform,_Standard_Edition){.http}, so it cannot be used on them either.
