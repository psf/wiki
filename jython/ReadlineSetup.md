...
1.  [Configure Readline for Jython 2.5](#Configure_Readline_for_Jython_2.5)
2.  [Configuring Readline for Jython 2.1](#Configuring_Readline_for_Jython_2.1)
3.  [Common Error Messages](#Common_Error_Messages)
4.  [Enhancements](#Enhancements)
5.  [Readline Alternatives](#Readline_Alternatives)
6.  [References](#References)
:::

### Configure Readline for Jython 2.5 {#Configure_Readline_for_Jython_2.5}

Jython now supports readline functionality of out the box, as of 2.5.0, by bundling JLine. In addition, 2.5.2 beta 2 adds completion support. See the [tracking issue](http://bugs.jython.org/issue1133){.http}.

You can also try [Jython Console](http://code.google.com/p/jythonconsole/){.http}, which is implemented on Swing. It features code completion and has been updated to work with Jython 2.5.

### Configuring Readline for Jython 2.1 {#Configuring_Readline_for_Jython_2.1}

(This content will moved to another page shortly and is probably of historical interest only at this point.)

Jython can support line editing and history via the [Java-Readline](http://java-readline.sourceforge.net/){.http} library on unix systems. Java-Readline wraps [GNU Readline](http://directory.fsf.org/readline.html){.http} with a JNI interface to provide advanced editing support to Java console programs. However, Java-Readline is not bundled with the standard Jython distribution. This note describes how to configure Readline support into an existing Jython install. It is not necessary to rebuild Jython from source, but you will need to build/install Java-Readline if a prebuilt version doesn\'t exist for your platform/OS distribution (which is likely the case).
...
1\. [Checkout Java-Readline](http://sourceforge.net/cvs/?group_id=48669){.http}, uncomment the correct OS_FLAVOR line in java-readline/Makefile and follow the included build instructions. This will require both java and C compilers. If the necessary compilers can be found in your path, building should be as simple as:
...
2\. Jython needs to be configured to use the Java-Readline library. Edit the jython [registry](http://www.jython.org/archive/21/docs/registry.html){.http} file, setting the following two lines:
...
### Common Error Messages {#Common_Error_Messages}
...
``` {#CA-7057eddedbdf673d7984a662dbaa731365b131b1 dir="ltr" lang="en"}
>>>import sys
>>>print sys.registry
```
:::
...
A [patch](http://sourceforge.net/tracker/index.php?func=detail&aid=795831&group_id=12867&atid=312867){.http} on the jython patch-tracker implements TAB support for indentation in ReadlineConsole. However, it has not yet been accepted into Jython.

### Readline Alternatives {#Readline_Alternatives}
...
- [rlwrap](http://utopia.knoware.nl/~hlub/uck/rlwrap/){.http} which also wraps GNU Readline, but can be used by invoking jython with:
...
- [jline](http://jline.sourceforge.net/){.http} is a java console command editor distributed in the form of a jar file. It can be used as a wrapper by putting the jline jar file in the CLASSPATH, and changing jython invokation to:
...
Note that jline can also be used programically to provide an alternative jython console, as in [JLineConsole](http://thread.gmane.org/gmane.comp.lang.jython.devel/713){.http}.

There are several Jython swing console\'s in various states of completion that provide command editing features in a cross-platform way. The simplest is the demo console bundled with jython in the [Demo/swing/Console.py](http://cvs.sourceforge.net/viewcvs.py/jython/jython/Demo/swing/Console.py?rev=1.1&view=auto){.http} subdir which provides a command history via the cursor-up and cursor-down keys. Others GUI console\'s include:

- [Jython Console](http://don.freeshell.org/jython/){.http} featuring code completion.

- The interstingly named [YaTiSeWoBe](http://www-iis.unil.ch/~jiglesia/apps/yatisewobe/doc/html/sec-cons.html){.http} has a Jython console.

- [jydle](http://cvs.sourceforge.net/viewcvs.py/jydle/Jydle/Console/){.http} is an attempt at an IDLE-like environment for Jython, and includes an interactive console.

- [jyleo](http://webpages.charter.net/edreamleo/front.html){.http} contains JythonShell. This appears to be actively developed but requires the alpha jython 2.2 release.

### References {#References}

The Java-Readline library includes [contributed instructions](http://cvs.sourceforge.net/viewcvs.py/java-readline/java-readline/contrib/jpython/jython-install.txt?rev=1.1&view=markup){.http} on building Jython with Readline support that have been very useful in sorting the process out. Many thanks to the author SteveC.

Charlie Moad [summarizes](http://article.gmane.org/gmane.comp.lang.jython.user/4018){.http} setup instructions on ubuntu using a pre-built Java-Readline package, as well as mentioning the CLASSPATH perils of invoking Jython using `java -jar`.

The Readline wrapper rlwrap was recommended by [Dave Kuhlman](http://article.gmane.org/gmane.comp.lang.jython.user/4245){.http}.

The use of JLine and the JLineConsole implementation itself are due to [Robert Andre](http://thread.gmane.org/gmane.comp.lang.jython.devel/713){.http}.

Don Coleman introduced his Jython Console with [this](http://thread.gmane.org/gmane.comp.lang.jython.user/779){.http} post to the jython-user mailing list.

[YaTiSeWoBe](./YaTiSeWoBe.html){.nonexistent} is due to [Javier Iglesias](http://article.gmane.org/gmane.comp.lang.jython.user/2787){.http}.

Jydle owes its development to [Michel Pelletier](http://article.gmane.org/gmane.comp.lang.jython.user/1001){.http}.

JythonShell was [introduced](http://thread.gmane.org/gmane.comp.lang.jython.user/4177){.http} by the enigmatic \"Leo User\".
::::::
