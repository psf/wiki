...
- Write a POJO (Plain Old Java Object see [http://en.wikipedia.org/wiki/POJO](http://en.wikipedia.org/wiki/POJO){.http}) with the same API as the Python module.

- Write a subclass of [PyObject](./PyObject.html){.nonexistent}.
...
The advantage of writing the class as a [PyObject](./PyObject.html){.nonexistent} subclass is all of the dynamic nature of Python is available for use. For example, in Python it\'s possible to call a method with keyword arguments. This is not possible in Java and therefore the use of this feature is only available if the class implements [PyObject](./PyObject.html){.nonexistent}.

### Approach {#Approach}

My original approach was to write as much of the class in Java as possible. This meant I always subclassed [PyObject](./PyObject.html){.nonexistent} and spent the effort to write all the marshalling code in Java to make what would essentially be a POJO act as [PyObject](./PyObject.html){.nonexistent}. This results in a lot of work but it does offer the greatest amount of flexibility and speed.
