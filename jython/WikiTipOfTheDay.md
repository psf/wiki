...
In your user preferences you can use regular expressions for subscribing pages; with "`.*`{.backtick}" you subscribe to all pages.
:::
::::

- WikiWiki means "fast" in Hawaiian.

- A [WikiName](WikiName) is a word composed of two or more words with each separate word beginning with an upper case character.

- Always use the singular for a [WikiName](WikiName), otherwise multiple pages for one topic might be created.

- Be bold when updating pages.---Back ups from older versions of each page are created automatically.

- By setting a bookmark on [RecentChanges](RecentChanges) you can easily see what has been changed since your last visit.

- [CategoryCategory](CategoryCategory) shows an overview of all available categories.
...
- For your account name you should use a [WikiName](WikiName) (FirstnameLastname).
...
- [OrphanedPages](OrphanedPages) shows pages that no other page links to by a [WikiName](WikiName).
...
- The use of [categories](CategoryCategory) helps organize your information.

- Use [WikiName](WikiName)s instead of URLs when linking to a page within the wiki.

- Use *Exclude underlay* on [FindPage](FindPage) to exclude the wiki\'s help pages from the fulltext search.

- [WantedPages](WantedPages) shows [WikiName](WikiName)s where currently no page exists.

- When writing your email address, use `<<MailTo(Firstname DOT Lastname AT example DOT net)>>`{.backtick} to avoid spam.

- Wikis are cool---especially if you have read the help pages (see [HelpContents](HelpContents)). ![;-)](/wiki/modernized/img/smile4.png ";-)"){height="16" width="16"}

- With `{{attachment:myfile.png}}`{.backtick} you can create a link to an attachment (and use the link to upload the attachment).

- With `{{attachment:myfile.png|alias description|&do=get}}`{.backtick} you can create a download link to an attachment.
...
- You can avoid that a [WikiName](WikiName) becomes a link if you place a `!`{.backtick} in front of the word.
...
- You can use other pages than the [FrontPage](FrontPage) as your personal start page, e.g. [RecentChanges](RecentChanges) or your own homepage.
