...
::: {#svn-to-hg-migration .section}
### Svn to Hg Migration

The main Jython repo is hosted at:

[http://hg.python.org/jython](http://hg.python.org/jython){.http .reference .external}

The checkins mailing list has also moved from SF.net to python.org:

[http://mail.python.org/mailman/listinfo/jython-checkins](http://mail.python.org/mailman/listinfo/jython-checkins){.http .reference .external}

You should read the CPython devguide, we\'ll for the most part follow their mercurial workflow:

[http://docs.python.org/devguide/](http://docs.python.org/devguide/){.http .reference .external}

We have a bitbucket mirror setup at [http://bitbucket.org/jython](http://bitbucket.org/jython){.http .reference .external} to ease forks (and pull requests).
...
::: {#where-d-my-feature-branch-go .section}
### Where\'d my feature branch go?

Another read-only repo includes all unmerged feature branches from subversion, located at:

[http://hg.python.org/jython-fullhistory](http://hg.python.org/jython-fullhistory){.http .reference .external}
...
TODO: mention how to merge from default to aid in later merges ( [http://codespeak.net/svn/pypy/extradoc/planning/hg-migration/active-branches-howto.txt](http://codespeak.net/svn/pypy/extradoc/planning/hg-migration/active-branches-howto.txt){.http .reference .external} )
:::
...
::: {#svn-to-hg-map .section}
### Svn to Hg Map

[svnmap.txt](attachments/SvnToHgMigration/jython-svnmap.txt){.attachment .reference .external} contains a mapping of original svn revision (and its original subversion branch name) to its hg revision.
...
from: [http://bitbucket.org/pjenvey/pymigr](http://bitbucket.org/pjenvey/pymigr){.http .reference .external}

These need to be ran with a modified version of hgsubversion, from here (with all three mq patches applied):

[http://bitbucket.org/pjenvey/hgsubversion-jython-conv](http://bitbucket.org/pjenvey/hgsubversion-jython-conv){.http .reference .external}
