...
There are many features within the JDK 6 builds which are going to open new doors for Java programmers. Perhaps one of the most unique new features of JDK 6 is the inclusion of a scripting project which allows usage of countless java scripting languages within Java applcations. Sun\'s implementation of Java SE6 includes a script engine based upon [Rhino: JavaScript for Java](http://www.mozilla.org/rhino){.http}. However, this scripting framework has been implemented in such a way that it also supports third-party engines that implement [JSR 223 Scripting APIs](http://download.java.net/jdk6/docs/api/javax/script/package-summary.html){.http}.

Of course, the topic for this article is the utilization of Jython within a Java application with the new JDK 6 scripting framework. The jython engine is available for download along with the other scripting language implementations on the [scripting.java.net website](https://scripting.dev.java.net/){.https}. While the Jython scripting engine only supports release 2.1, it\'s utilization provides seamless integration within Java applications\...with a minor effort.

## Setting Up The Environment {#Setting_Up_The_Environment}

To begin using the JDK 6 scripting engine, you will need to obtain a copy of JDK6. You can use [this link](http://java.sun.com/javase/downloads/ea.jsp){.http} to grab the latest release as of this writing. You\'ll also need to visit the [scripting.java.net website](https://scripting.dev.java.net/){.https} mentioned previously and obtain a copy of the JSR 223 engines.
...
## IDE Recommendation {#IDE_Recommendation}

In order to facilitate the usage of Jython within your Java application, I recommend using an IDE which provides some support for the Jython language. For instance, I use Netbeans along with the [Coyote](https://coyote.dev.java.net/){.https} plugin. You may prefer to use Eclipse or something else as it is personal preference. Personally, I find it cumbersome to go from a Java IDE to a text editor for writing Jython scripts.

## Utilizing Jython from Java {#Utilizing_Jython_from_Java}
...
As stated earlier, the scripting API has support for a number of languages and this is orchestrated via the the [ScriptEngineManager](./ScriptEngineManager.html){.nonexistent}. In order to select a specified scripting engine to use, you\'ll first need to obtain the scripting engine from the [scripting.java.net website](https://scripting.dev.java.net/){.https}. Once you\'ve extracted the engine of your choice, in our case Jython, you\'ll have to ensure that the engine resides within your classpath. Once this task has been completed, it is easy to gain access to your scripting language.
...
[Scripting for the Java Platform](http://developers.sun.com/learning/javaoneonline/2006/coreplatform/TS-1382.pdf){.http}

[JRunscript Command Line script Shell](http://java.sun.com/javase/6/docs/technotes/tools/share/jrunscript.html){.http}

[JDK 6 Project](https://jdk6.dev.java.net/){.https}

[scripting.dev.java.net](https://scripting.dev.java.net/){.https}
:::
