...
## Invoking the Interpreter {#Invoking_the_Interpreter}
...
## Loading the Jython Plugin at Startup {#Loading_the_Jython_Plugin_at_Startup}

In the previous code snippet, you might have noticed that we didn\'t just invoke the Jython Interpreter directly, we extracted an already instantiated instance from the [ServletContext](./ServletContext.html){.nonexistent}. We created a Jython Struts plugin that can load a Jython Interpreter instance at startup which we can get at a later time to avoid having to reload the object for every request. It is configured to launch at startup in the \'struts-config.xml\' file.
...
## More on the Web Application Configuration including struts-config.xml {#More_on_the_Web_Application_Configuration_including_struts-config.xml}
...
## More on the Struts web-application {#More_on_the_Struts_web-application}
...
When the user hits the submit button, they are invoking our [DefaultActionHandler](./DefaultActionHandler.html){.nonexistent} which in turn invokes the Jython Action class that contains our Hibernate persistance code.

## Jython and Hibernate {#Jython_and_Hibernate}
...
## The Hibernate Plugin {#The_Hibernate_Plugin}

Like the Struts plugin mentioned previously, the Hibernate plugin loads an instance of the Hibernate [SessionFactory](./SessionFactory.html){.nonexistent} and saves it for later use through out the application.
...
## Jython and Hibernate CRUD Operations {#Jython_and_Hibernate_CRUD_Operations}

We discussed earlier how Jython is basically used for the backend coding, that includes communicating with Hibernate. Here are the code snippets associated with each of those operations. Most of the code is fairly intuitive; at the heart of the create operation, you must get the Hibernate [SessionFactory](./SessionFactory.html){.nonexistent} and initiate a transaction. Once that is done, create an instance of the Hibernate POJO bean and populate the bean with the data from the Struts [ActionForm](./ActionForm.html){.nonexistent}. Once that is taken care of, use the session and transaction object to save the data. The Edit operation probably contains the most code and is seperated into two Jython classes.
...
The only reference to the Jython installation is through the hardcoded member in the Plugin class, [JythonUtilPlugin](./JythonUtilPlugin.html){.nonexistent}. Point this value to your installation.
...
Once you have deployed your application, point your web browser to: [http://localhost:8080/botspiritlistbeta/](http://localhost:8080/botspiritlistbeta/){.http}
...
## Summary for Running the Example Application {#Summary_for_Running_the_Example_Application}
...
[Hibernate Mapping Config File](http://www.newspiritcompany.com/src/jytut/Botlist.hbm.xml.html){.http}.

[Hibernate Config File](http://www.newspiritcompany.com/src/jytut/hibernate.cfg.xml.html){.http}.

[Struts Configuration file](http://www.newspiritcompany.com/src/jytut/struts-config.xml.html){.http}.

[Example List JSP Page](http://www.newspiritcompany.com/src/jytut/ListLinks.jsp.html){.http}.

[List Jython Action Class](http://www.newspiritcompany.com/src/jytut/ListLinksAction.py.html){.http}.

[Create Jython Action Class](http://www.newspiritcompany.com/src/jytut/SimpleStrutsAction.py.html){.http}.

[View Jython Action Class](http://www.newspiritcompany.com/src/jytut/ViewAction.py.html){.http}.

[Full Zipped Source](http://www.newspiritcompany.com/src/jytut/BotSpiritListExample.zip){.http}.

## Resources {#Resources}

[Jakarta Struts Home](http://struts.apache.org/){.http}

[Hibernate Home](http://www.hibernate.org/){.http}

[Jython Home](http://www.jython.org){.http}

[MySQL Home, Download the Java Connector](http://www.mysql.com/){.http}

[Jython Web Application From Sean McGrath](http://seanmcgrath.blogspot.com/JythonWebAppTutorialPart1.html){.http}

[Tomcat Servlet Container](http://tomcat.apache.org/){.http}
