...
# adding JARs to sys.path at runtime {#adding_JARs_to_sys.path_at_runtime}
...
\"What?\", you ask. Well, in my case I came upon this problem in the following way. I am working on an open source IHE Image Archive Actor \[1\] and needed a web interface. I\'m using AJAX on the client side to route database calls through CGI to a jython-JDBC enabled API. Testing the jython-JDBC API from the command line worked fine, I had the [PostGres](./PostGres.html){.nonexistent} driver in my CLASSPATH. But, when called via the web interface I got \"zxJDBC error, postGres driver not found\" errors. Why? Because APACHE was calling the API and APACHE is not a normal account with environment variables.
...
1\. Information on IHE, [http://www.ihe.net/Resources/index.cfm](http://www.ihe.net/Resources/index.cfm){.http}

2\. Samuele Pedroni, Noel Rappin. (2002). \"Jython Essentials\", O\'Reilly Publishing, [http://www.ora.com](http://www.ora.com){.http}

3\. The original Java classPathHacker, [http://forum.java.sun.com/thread.jspa?threadID=300557](http://forum.java.sun.com/thread.jspa?threadID=300557){.http}
:::
