...
The podcast is will be available on April 4th: [http://www.jythonpodcast.com](http://www.jythonpodcast.com){.http}.
...
Sqlalchemy 0.6 has been released with su port for jython 2.5.  [Philip Jenvey blogs](http://dunderboss.blogspot.com/2010/04/sqlalchemy-06-released-with-jython.html){.http} that SQLAlchemy on Jython uses JDBC drivers via Jython\'s builtin zxJDBC DBAPI interface. It currently supports PostgreSQL, MySQL, Oracle and also MS SQL Server via jTDS.
...
[How to install Python Libraries](http://freecog.com/?p=195){.http} - [FreeCog](./FreeCog.html){.nonexistent}
...
[Intercepting Java method invocations in Jython](http://slava-technical.blogspot.com/2010/04/when-ive-created-latest-version-of.html){.http} - Borislav Andrschuk
...
[Options for WebSphere App Server Scripting](http://webspherecommunity.blogspot.com/2010/04/options-for-websphere-application.html){.http} - WS Community Blog

The Websphere community blog had a post in April regarding options for web sphere scripting.    [WebSphere](WebSphere) Application Server (WAS) provides a scripting interface called wsadmin. wsadmin supports two scripting languages jacl\* and jython. Five objects are available when you use scripts: [AdminControl](./AdminControl.html){.nonexistent}: Use to run operational commands. [AdminConfig](./AdminConfig.html){.nonexistent}: Use to run configurational commands to create/modify WAS configuration. [AdminApp](./AdminApp.html){.nonexistent}: Use to administer applications. [AdminTask](./AdminTask.html){.nonexistent}: Use to run administrative commands. Help: Use to obtain general help.
...
[Django vs Grails](http://western-skies.blogspot.com/2010/04/django-vs-grails.html){.http} - Western Skies - Charles Anderson
...
[Back To Jython DB4O](http://jimcassidy.ca/2010/04/19/back-to-jythondb4o-part-1/){.http} - Jim Cassidy
...
[Scripting from Scratch:  Creating a Jython administrative script for websphere](http://public.dhe.ibm.com/software/dw/websphere/1004_gibson-pdf.pdf){.http} - Bob Gibson
...
[PyDev 1.5.6](http://pydev.sourceforge.net/){.http} - Now with Django Integration!
